...
//  Built-in actions for interstitial polish (no workflow dependency)
//

import Foundation

/// A smart action with an icon, name, and detailed prompt template
public struct SmartAction: Identifiable, Equatable, Sendable {
    public let id: String
    public let name: String
    public let icon: String
    public let defaultPrompt: String

    public init(id: String, name: String, icon: String, defaultPrompt: String) {
        self.id = id
        self.name = name
        self.icon = icon
    ...
    public static func == (lhs: SmartAction, rhs: SmartAction) -> Bool {
        lhs.id == rhs.id
    }

    /// Built-in smart actions with curated prompt templates
    public static let builtIn: [SmartAction] = [
        SmartAction(
            id: "fix_grammar",
            name: "Fix Grammar",
            icon: "textformat.abc",
    ...
    public static func action(id: String) -> SmartAction? {
        builtIn.first { $0.id == id }
    }

    /// Get all built-in actions (for TalkieAgent interstitial, no workflow access)
    public static func allBuiltIn() -> [SmartAction] {
        builtIn
    }
}
