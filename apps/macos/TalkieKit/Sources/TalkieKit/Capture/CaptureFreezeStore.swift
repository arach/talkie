...
//  prime a full-display still before the overlay or HUD bar appears, then both
//  paint it under the crosshair and crop the saved image from it. Transient
//  windows that would vanish on focus change survive in the still.
//
...
public final class CaptureFreezeStore {
    ...
    /// still on screen. The actual pixel grab is awaited before the overlay
    /// takes key focus, guaranteeing the menu/popover is captured.
    ...
    /// the same primed still; the underlying grab runs once.
    ...
    /// the overlay's screen. Mismatch returns nil so the overlay keeps live
    /// behavior.
    ...
    /// Returns nil when there is no still, the screen does not match, or the
    /// rect falls outside the still.
    public func crop(screenRect: CGRect) async -> CGImage? {
        ...
        // Cocoa (bottom-left) to image (top-left). Same flip captureScreenRegion
    ...
    private static func capture(for screen: NSScreen) async -> Frozen? {
        ...
        do {
            ...
            // so an already-visible bar is not baked into the still even when
            // the grab lands after the bar appears. Layer 0 app windows stay.
