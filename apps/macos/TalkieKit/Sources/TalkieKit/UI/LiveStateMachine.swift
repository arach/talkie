...
//  TalkieKit
//
//  Robust state machine for TalkieAgent recording states
...
public enum LiveStateEvent {
    ...
    case beginRouting
    case beginRefining    // Visual transition: routing → refining (LLM call)
    case complete
    case cancel
    case error(String)
    case forceReset
...
public final class LiveStateMachine: ObservableObject {
    ...
    private func validTransition(from currentState: LiveState, event: LiveStateEvent) -> LiveState? {
        ...
        case (.routing, .beginRefining):
            return .refining  // Visual transition to refining sub-phase
        ...
        // REFINING state transitions (mirrors routing — it's a visual sub-phase)
        case (.refining, .complete):
            return .idle
        case (.refining, .cancel):
            return .idle
        case (.refining, .error):
            return .idle
        case (.refining, .forceReset):
            return .idle
...
/*
 ...
 ROUTING
  ├─ beginRefining → REFINING (visual sub-phase for LLM call)
