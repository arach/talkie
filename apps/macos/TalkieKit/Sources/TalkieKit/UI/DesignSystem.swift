...
private struct ThemeConfigState {
    var cornerRadiusMultiplier: CGFloat = 1.0
    var useLightFonts = false
    var borderWidth: CGFloat = 1.0
    var customFontName: String?
}

private struct CornerRadiusState {
    var xs: CGFloat = 4
    var sm: CGFloat = 8
    var md: CGFloat = 12
    var lg: CGFloat = 16
    var xl: CGFloat = 24
    var card: CGFloat = 12
    var cardSmall: CGFloat = 8
    var cardLarge: CGFloat = 16
}

private struct BorderWidthState {
    var thin: CGFloat = 0.5
    var normal: CGFloat = 1.0
    var thick: CGFloat = 2.0
}
...
public enum ThemeConfig {
    private static let storage = LockedBox(ThemeConfigState())

    /// Corner radius multiplier (stored for reference)
    public static var cornerRadiusMultiplier: CGFloat {
        storage.read(\.cornerRadiusMultiplier)
    }

    /// Whether to use light/thin font weights
    public static var useLightFonts: Bool {
        storage.read(\.useLightFonts)
    }

    /// Border width (calculated once)
    public static var borderWidth: CGFloat {
        storage.read(\.borderWidth)
    }

    /// Custom font name override (e.g., "JetBrainsMono" for Terminal theme)
    public static var customFontName: String? {
        storage.read(\.customFontName)
    }
    ...
    public static func configure(
        ...
        storage.withValue { state in
            state.cornerRadiusMultiplier = cornerRadiusMultiplier
            state.useLightFonts = useLightFonts
            state.borderWidth = borderWidth
            state.customFontName = customFontName
        }
...
public enum CornerRadius {
    private static let storage = LockedBox(CornerRadiusState())
    ...
    public static var xs: CGFloat { storage.read(\.xs) }
    /// Small (8pt base, theme-adjusted)
    public static var sm: CGFloat { storage.read(\.sm) }
    /// Medium (12pt base, theme-adjusted)
    public static var md: CGFloat { storage.read(\.md) }
    /// Large (16pt base, theme-adjusted)
    public static var lg: CGFloat { storage.read(\.lg) }
    /// Extra large (24pt base, theme-adjusted)
    public static var xl: CGFloat { storage.read(\.xl) }

    // MARK: - Semantic Aliases (for cards, buttons, modals)
    // These map to size values for semantic clarity

    /// Card corner radius (maps to md, 12pt base) - use for all cards
    public static var card: CGFloat { storage.read(\.card) }
    /// Small card/button corner radius (maps to sm, 8pt base)
    public static var cardSmall: CGFloat { storage.read(\.cardSmall) }
    /// Large card/modal corner radius (maps to lg, 16pt base)
    public static var cardLarge: CGFloat { storage.read(\.cardLarge) }

    /// Recalculate all cached values. Called when theme changes.
    public static func recalculate(multiplier: CGFloat) {
        storage.withValue { state in
            state.xs = baseXS * multiplier
            state.sm = baseSM * multiplier
            state.md = baseMD * multiplier
            state.lg = baseLG * multiplier
            state.xl = baseXL * multiplier

            // Update semantic aliases
            state.card = state.md
            state.cardSmall = state.sm
            state.cardLarge = state.lg
        }
    }
...
public enum BorderWidth {
    private static let storage = LockedBox(BorderWidthState())
    ...
    public static var thin: CGFloat { storage.read(\.thin) }
    /// Normal border (1pt base)
    public static var normal: CGFloat { storage.read(\.normal) }
    /// Thick border (2pt base)
    public static var thick: CGFloat { storage.read(\.thick) }

    /// Recalculate cached values. Called when theme changes.
    public static func recalculate(multiplier: CGFloat) {
        storage.withValue { state in
            state.thin = baseThin * multiplier
            state.normal = baseNormal * multiplier
            state.thick = baseThick * multiplier
        }
    }
}
...
/// Standard border opacity values for consistent card/component styling
/// These are constants (not theme-dependent) as opacity is about visibility, not theme style
public enum BorderOpacity {
    /// Subtle border - barely visible (0.06)
    public static let subtle: Double = 0.06
    /// Light border - visible but understated (0.10)
    public static let light: Double = 0.10
    /// Standard border - clear definition (0.15)
    public static let standard: Double = 0.15
    /// Strong border - prominent (0.20)
    public static let strong: Double = 0.20
    /// Hover state border (0.25)
    public static let hover: Double = 0.25
}
...
public enum GlassConfig {
    ...
    public enum DebugRenderMode: Sendable {
    ...
    public static var debugRenderMode: DebugRenderMode {
        get { storage.read(\.debugRenderMode) }
        set { storage.withValue { $0.debugRenderMode = newValue } }
    }
...
enum SimpleGlassFallbackPalette {
    static var cardFill: NSColor { .controlBackgroundColor }
    static var cardStroke: NSColor { dynamicStroke(opacity: BorderOpacity.standard) }

    static var pillFill: NSColor { .controlBackgroundColor }
    static var pillStroke: NSColor { dynamicStroke(opacity: BorderOpacity.light) }

    private static func dynamicStroke(opacity: Double) -> NSColor {
        NSColor(name: nil) { appearance in
            let isDarkAppearance = appearance.bestMatch(from: [.aqua, .darkAqua]) == .darkAqua
            let baseColor = isDarkAppearance ? NSColor.white : NSColor.black
            return baseColor.withAlphaComponent(CGFloat(opacity))
        }
    }
}
...
public struct LiquidGlassCardModifier: ViewModifier {
    ...
    public func body(content: Content) -> some View {
        if GlassConfig.shouldUseSimpleMode {
            let fill = fallbackFill ?? Color(SimpleGlassFallbackPalette.cardFill)
            let stroke = fallbackStroke ?? Color(SimpleGlassFallbackPalette.cardStroke)
            ...
            content
                .background(
                    RoundedRectangle(cornerRadius: cornerRadius)
                        ...
                        .overlay(
                            RoundedRectangle(cornerRadius: cornerRadius)
                                .stroke(stroke, lineWidth: BorderWidth.thin)
                        )
                )
...
public struct LiquidGlassPillModifier: ViewModifier {
    ...
    public func body(content: Content) -> some View {
        if GlassConfig.shouldUseSimpleMode {
            let fill = fallbackFill ?? Color(SimpleGlassFallbackPalette.pillFill)
            let stroke = fallbackStroke ?? Color(SimpleGlassFallbackPalette.pillStroke)
...
/// Unified hover card modifier for consistent card interactions across Talkie
/// Provides scale + border brightness animation on hover
public struct HoverCardModifier: ViewModifier {
    var cornerRadius: CGFloat
    var fillColor: Color
    var borderBaseOpacity: Double
    var borderHoverOpacity: Double
    ...
    public init(
        cornerRadius: CGFloat = CornerRadius.md,
        fillColor: Color = TalkieTheme.surface,
        borderBaseOpacity: Double = 0.08,
        borderHoverOpacity: Double = 0.2,
        scaleEffect: CGFloat = 1.02,
        animation: Animation = .spring(response: 0.3, dampingFraction: 0.7)
    ) {
        self.cornerRadius = cornerRadius
        self.fillColor = fillColor
        self.borderBaseOpacity = borderBaseOpacity
        self.borderHoverOpacity = borderHoverOpacity
            ...
            .background(
                RoundedRectangle(cornerRadius: cornerRadius)
                    ...
                    .overlay(
                        RoundedRectangle(cornerRadius: cornerRadius)
                            .strokeBorder(
                                TalkieTheme.border.opacity(isHovered ? borderHoverOpacity : borderBaseOpacity),
                                lineWidth: 1
                            )
                    )
            )
...
/// Hover card modifier with external state control
public struct HoverCardExternalModifier: ViewModifier {
    var isHovered: Bool
    var cornerRadius: CGFloat
    var fillColor: Color
    var borderBaseOpacity: Double
...
public extension View {
    /// Apply hover card effect with automatic hover detection
    func hoverCard(
        cornerRadius: CGFloat = CornerRadius.md,
        fillColor: Color = TalkieTheme.surface,
        borderBaseOpacity: Double = 0.08,
        ...
        modifier(HoverCardModifier(
            cornerRadius: cornerRadius,
            fillColor: fillColor,
            borderBaseOpacity: borderBaseOpacity,
            borderHoverOpacity: borderHoverOpacity,
            scaleEffect: scaleEffect
        ))
    }

    /// Apply hover card effect with external hover state
    func hoverCard(
        isHovered: Bool,
        cornerRadius: CGFloat = CornerRadius.md,
        fillColor: Color = TalkieTheme.surface,
        ...
        modifier(HoverCardExternalModifier(
            isHovered: isHovered,
            cornerRadius: cornerRadius,
            fillColor: fillColor,
            borderBaseOpacity: borderBaseOpacity,
