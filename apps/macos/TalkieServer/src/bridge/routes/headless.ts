 ...
 *
 * Uses Claude CLI directly (bypasses Talkie/TalkieAgent):
...
export async function headlessRoute(body: HeadlessRequest): Promise<Response> {
  ...
  log.info(`Headless: sessionId=${sessionId || "(new)"}, projectDir=${workingDir || "not set"}, message=${message.slice(0, 100)}...`);

  // Build CLI command — resume existing session or fire one-shot
  const args: string[] = [];
  if (sessionId) {
