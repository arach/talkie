/**
 * Device Management Routes
 *
 * GET    /devices          - List all paired devices
 * DELETE /devices/:id      - Remove a specific device
 * DELETE /devices          - Remove all devices (revoke all)
 */

import {
  type DeviceSetupState,
  getDeviceSummary,
  removeDevice,
  revokeAllDevices,
  updateDeviceSetupState,
} from "../../devices/registry";
import { log } from "../../log";
import { notFound } from "./responses";

// ===== Types =====

export interface DeviceInfo {
  id: string;
  name: string;
  pairedAt: string;
  lastSeen: string | null;
  isExpired: boolean;
  daysUntilExpiry: number | null;
  setupState: DeviceSetupState | null;
}

export interface DevicesResponse {
  total: number;
  active: number;
  expired: number;
  devices: DeviceInfo[];
}

export interface DeviceRemoveResponse {
  success: boolean;
  message: string;
}

export interface DevicesRevokeResponse {
  success: boolean;
  count: number;
  message: string;
}

export interface DeviceSetupStateRequest {
  followComputerShortcutMode?: boolean;
  companionSurfaceActive?: boolean;
  terminalImported?: boolean;
  terminalHost?: string | null;
}

export interface DeviceSetupStateResponse {
  success: boolean;
}

// ===== Handlers =====

/**
 * GET /devices
 * List all paired devices with status info
 */
export async function devicesListRoute(): Promise<DevicesResponse> {
  const summary = await getDeviceSummary();

  log.info(`Devices: ${summary.active} active, ${summary.expired} expired`);

  return summary;
}

/**
 * POST /devices/setup-state
 * Persist a lightweight setup snapshot for the authenticated device.
 */
export async function deviceSetupStateRoute(
  deviceId: string | null,
  body: DeviceSetupStateRequest
): Promise<DeviceSetupStateResponse | Response> {
  if (!deviceId) {
    return new Response("Missing device identity", { status: 401 });
  }

  const updated = await updateDeviceSetupState(deviceId, {
    followComputerShortcutMode: body.followComputerShortcutMode === true,
    companionSurfaceActive: body.companionSurfaceActive === true,
    terminalImported: body.terminalImported === true,
    terminalHost: body.terminalHost ?? null,
  });

  if (!updated) {
    return notFound("Device not found");
  }

  log.info(
    `Updated setup state for ${updated.name}: shortcutFollow=${updated.setupState?.followComputerShortcutMode === true} terminalImported=${updated.setupState?.terminalImported === true}`
  );

  return { success: true };
}

/**
 * DELETE /devices/:id
 * Remove a specific paired device
 */
export async function deviceRemoveRoute(deviceId: string): Promise<DeviceRemoveResponse | Response> {
  const removed = await removeDevice(deviceId);

  if (!removed) {
    return notFound("Device not found");
  }

  log.info(`Removed device: ${deviceId}`);

  return {
    success: true,
    message: "Device removed",
  };
}

/**
 * DELETE /devices
 * Revoke all paired devices
 */
export async function devicesRevokeAllRoute(): Promise<DevicesRevokeResponse> {
  const count = await revokeAllDevices();

  return {
    success: true,
    count,
    message: count > 0 ? `Revoked ${count} device(s)` : "No devices to revoke",
  };
}
