...
import {
  ...
  deriveAuthKeyFromBase64,
  deriveEncryptionKeyFromBase64,
} from "../crypto/keypair";
...
export async function updateDeviceSetupState(
  deviceId: string,
  setupState: Omit<DeviceSetupState, "reportedAt">
  ...
  device.setupState = {
    ...device.setupState,
    ...setupState,
    terminalHost: setupState.terminalHost ?? null,
...
/**
 * Get the AES-GCM encryption key for a paired device (for encrypted payloads)
 * Uses HKDF with info="talkie-bridge-encrypt"
 * Returns null for expired devices (must re-pair)
 */
export async function getDeviceEncryptionKey(deviceId: string): Promise<CryptoKey | null> {
  const devices = await getDevices();
  const device = devices.find((d) => d.id === deviceId);
  if (!device) {
    return null;
  ...
  const serverKeyPair = await getOrCreateKeyPair();
  return deriveEncryptionKeyFromBase64(serverKeyPair.privateKey, device.publicKey);
}
