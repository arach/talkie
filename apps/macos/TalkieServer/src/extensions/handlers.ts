...
import {
  ...
  transcribeStart,
  transcribeStop,
  clipboardWrite as headlessClipboardWrite,
  clipboardRead as headlessClipboardRead,
} from "./headless";
import type {
  Capability,
  CAPABILITIES,
...
// Route to TalkieHeadless which has XPC access to TalkieEngine

export async function handleTranscribePreflight(ctx: HandlerContext): Promise<void> {
  if (!hasCapability(ctx, "transcribe")) return;

  try {
    // Check if TalkieHeadless is available first
    const available = await isHeadlessAvailable();
    if (!available) {
      ctx.send({
        type: "transcribe:preflight",
        ready: false,
        checks: [
          { name: "TalkieHeadless", ok: false, detail: "Not running" },
          { name: "TalkieAgent", ok: false, detail: "Unknown (Headless unavailable)" },
          { name: "Microphone", ok: false, detail: "Unknown (Headless unavailable)" },
          { name: "TalkieEngine", ok: false, detail: "Unknown (Headless unavailable)" },
        ],
      });
      return;
    }

    const result = await transcribePreflight();
    log.info(`Extensions: Preflight complete - ready: ${result.ready}`);

    ctx.send({
      type: "transcribe:preflight",
      ready: result.ready,
      checks: [
        { name: "TalkieHeadless", ok: true, detail: "Running" },
        ...result.checks,
      ],
    });
  } catch (error) {
    log.error(`Extensions: Preflight failed: ${error}`);
    ctx.send({
      type: "transcribe:preflight",
      ready: false,
      checks: [
        { name: "TalkieHeadless", ok: false, detail: `Error: ${error}` },
      ],
    });
  }
}
...
export async function handleTranscribeStart(ctx: HandlerContext): Promise<void> {
  ...
  try {
    // Check if TalkieHeadless is available
    const available = await isHeadlessAvailable();
    if (!available) {
      log.warn("Extensions: TalkieHeadless not available");
      ctx.send({
        type: "error",
        error: "TalkieHeadless is not running. Launch TalkieHeadless.app first.",
        code: "HEADLESS_UNAVAILABLE",
      });
      return;
    }

    const result = await transcribeStart();
    log.info(`Extensions: Transcription started - session ${result.sessionId}`);
    ...
    log.error(`Extensions: Transcribe start failed: ${error}`);
    ctx.send({
      type: "error",
...
export async function handleTranscribeStop(ctx: HandlerContext): Promise<void> {
  ...
  try {
    ...
    log.info(`Extensions: Transcription complete - ${result.text.length} chars, ${result.duration}s`);
    ...
    log.error(`Extensions: Transcribe stop failed: ${error}`);
    ctx.send({
      type: "error",
      error: `Failed to stop transcription: ${error}`,
      code: "TRANSCRIBE_ERROR",
    });
  }
}
