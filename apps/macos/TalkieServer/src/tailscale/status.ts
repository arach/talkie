...
/**
 * Get the local Tailscale IPv4 address that the bridge should bind to.
 * Returns null if we can't determine a safe tailnet-only bind address.
 */
export async function getTailscaleBindAddress(): Promise<string | null> {
  const tailscalePath = findTailscalePath();
  if (tailscalePath) {
    const result = await $`${tailscalePath} status --json`.quiet().nothrow();
    if (result.exitCode === 0) {
      try {
        const status: TailscaleStatus = JSON.parse(result.stdout.toString());
        if (status.Self?.Online) {
        ...
        // Fall through to network interface detection.
      }
    }
  }

  return getTailscaleIPFromNetwork();
}
