...
struct Utterance: Identifiable, Codable, Hashable {
    ...
    /// Recording ID from unified database (UUID string)
    var recordingID: UUID?

    /// Legacy database ID (from LiveDictation) - deprecated, use recordingID
    var liveID: Int64?
    ...
    /// Initialize from database record (unified database)
    init(text: String, durationSeconds: Double?, metadata: DictationMetadata, timestamp: Date, recordingID: UUID) {
        self.id = UUID()
        self.text = text
    ...
    /// Initialize from database record (legacy)
...
struct ContextCapture {
    ...
    static let agentBundleID = Bundle.main.bundleIdentifier ?? "to.talkie.agent"

    /// Check if Talkie Agent is the frontmost app
    static func isTalkieAgentFrontmost() -> Bool {
        guard let frontApp = NSWorkspace.shared.frontmostApplication else { return false }
        return frontApp.bundleIdentifier == agentBundleID
    }
...
final class DictationStore {
    ...
    /// Add a new utterance (stores to unified recordings database)
    func add(_ text: String, durationSeconds: Double? = nil, metadata: DictationMetadata = DictationMetadata()) {
        // Create LiveRecording for unified database
        let recording = LiveRecording(
            ...
            mode: metadata.routingMode,
            browserURL: metadata.browserURL,
            documentURL: metadata.documentURL,
            terminalWorkingDir: metadata.terminalWorkingDir
        )
    ...
    /// Add a LiveDictation directly (for new recordings) - converts to LiveRecording
    func addLive(_ liveUtterance: LiveDictation) {
        let recording = LiveRecording(from: liveUtterance)
        UnifiedDatabase.store(recording)
    ...
    func update(_ utterance: Utterance) {
        // For now, just refresh - updates go through UnifiedDatabase directly
        refresh()
    }

    /// Delete an utterance
    func delete(_ utterance: Utterance) {
        // Use recordingID if available (unified database)
        if let recordingID = utterance.recordingID {
            UnifiedDatabase.delete(id: recordingID)
            logger.info("Deleted utterance by recording ID: \(utterance.text.prefix(30))...")
        } else {
            logger.warning("Cannot delete utterance without recordingID: \(utterance.text.prefix(30))...")
        }
        refresh()
    }
    ...
    /// Prune expired utterances and orphaned segments
    func pruneExpired() {
        UnifiedDatabase.pruneDictations(olderThanHours: ttlHours)
        UnifiedDatabase.pruneOrphanedSegments()
        refresh()
    }

    /// Refresh from database
    func refresh() {
        let recordings = UnifiedDatabase.recentDictations(limit: 500)
        utterances = recordings.map { recording in
            Utterance(
                text: recording.text,
        ...
        logger.info("🔄 DictationStore.refresh - loaded \(self.utterances.count) utterances from unified database")
    ...
    /// Build DictationMetadata from LiveRecording (unified database)
    private func buildMetadata(from recording: LiveRecording) -> DictationMetadata {
        var metadata = DictationMetadata()
        metadata.transcriptionModel = recording.transcriptionModel
        metadata.audioFilename = recording.hasAudio ? recording.audioFilename : nil
        ...
        if let json = recording.metadataJSON,
           ...
           let dict = try? JSONSerialization.jsonObject(with: data) as? [String: Any] {
            ...
            // App context
            if let app = dict["app"] as? [String: String] {
                metadata.activeAppBundleID = app["bundleId"]
                metadata.activeAppName = app["name"]
                metadata.activeWindowTitle = app["windowTitle"]
            }

            // Performance metrics
            if let perf = dict["performance"] as? [String: Any] {
                metadata.perfEngineMs = perf["engineMs"] as? Int
                metadata.perfEndToEndMs = perf["endToEndMs"] as? Int
                metadata.perfInAppMs = perf["inAppMs"] as? Int
                metadata.sessionID = perf["sessionId"] as? String
            }

            // Rich context
            if let context = dict["context"] as? [String: String] {
                metadata.browserURL = context["browserURL"]
                metadata.documentURL = context["documentURL"]
                metadata.terminalWorkingDir = context["terminalWorkingDir"]
            }

            // Routing info
            if let routing = dict["routing"] as? [String: String] {
                metadata.routingMode = routing["mode"]
            }
        }
    ...
    /// Build DictationMetadata from LiveDictation (legacy - for migration compatibility)
    ...
    /// Get total count
    var count: Int {
        UnifiedDatabase.countDictations()
    }

    /// Search utterances
    func search(_ query: String) -> [Utterance] {
        let recordings = UnifiedDatabase.searchDictations(query: query)
