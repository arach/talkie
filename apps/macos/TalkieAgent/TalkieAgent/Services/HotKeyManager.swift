...
private final class HotKeyRegistry {
    ...
    /// Stores the mach_absolute_time when the key was pressed so we can expire stale entries.
    /// A key held "down" for more than `staleKeyTimeout` is assumed to have lost its release event.
    private var keysDownSince: [HotKeyIdentifier: UInt64] = [:]

    /// Max time (in seconds) a key can be "down" before we assume the release was lost.
    /// Carbon release events can be dropped during app focus changes or audio error recovery.
    private let staleKeyTimeout: UInt64 = 3
    ...
    func register(signature: OSType, id: UInt32, onPress: @escaping (HotKeyTimestamp) -> Void, onRelease: (() -> Void)? = nil) {
        ...
        log.debug("Registered callback for signature=\(signature) id=\(id) hasRelease=\(onRelease != nil)")
    ...
    func handleEvent(signature: OSType, id: UInt32, eventType: HotKeyEventType, timestamp: HotKeyTimestamp) {
        ...
        case .pressed:
            ...
            // Safety: if the key has been "down" longer than staleKeyTimeout,
            // the release event was likely lost (can happen during audio errors,
            // app focus changes, etc.). Treat this press as fresh.
            if let downSince = keysDownSince[identifier] {
                let elapsedNs = mach_absolute_time() - downSince
                let elapsedMs = HotKeyTimestamp.ticksToMs(elapsedNs)
                if elapsedMs < Int(staleKeyTimeout * 1000) {
                    log.debug("Ignoring key repeat for signature=\(signature) id=\(id)")
                    return
                }
                log.warning("Stale key-down detected (\(elapsedMs)ms without release) — clearing stuck state for signature=\(signature) id=\(id)")
            }
            keysDownSince[identifier] = mach_absolute_time()

            if let callback = pressCallbacks[identifier] {
                log.info("Hotkey pressed: signature=\(signature) id=\(id)")
                callback(timestamp)
            } else {
                log.warning("No press callback registered for this hotkey")
            }
        case .released:
            keysDownSince.removeValue(forKey: identifier)

            if let callback = releaseCallbacks[identifier] {
                log.info("Hotkey released: signature=\(signature) id=\(id)")
                callback()
            }
    ...
    private func installHandlerIfNeeded() {
        ...
        log.info("Installed global event handler (press+release), status=\(status)")
    }
}
...
final class HotKeyManager {
    ...
    func registerHotKey(modifiers: UInt32, keyCode: UInt32, onPress: @escaping (HotKeyTimestamp) -> Void, onRelease: (() -> Void)?) {
        // Always clean up previous registration to prevent Carbon hotkey leaks.
        // Without this, calling registerHotKey twice orphans the old EventHotKeyRef
        // and the leaked registration can interfere with the new one.
        if hotKeyRef != nil {
            unregisterAll()
        }
        ...
        log.info("Registering hotkey: signature=\(self.signature)(\(sig)) id=\(self.hotkeyID) keyCode=\(keyCode) modifiers=\(modifiers) hasPTT=\(onRelease != nil)")
