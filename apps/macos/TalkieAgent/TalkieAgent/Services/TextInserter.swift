...
/// Centralized text insertion service for TalkieAgent
@MainActor
final class TextInserter {
    ...
    private func clipboardPaste(_ text: String) async -> Bool {
        ...
        // Clipboard is synchronous - paste immediately
    ...
    private func clipboardPasteWithFocus(_ text: String, app: NSRunningApplication) async -> Bool {
        ...
        // 1. Activate the app (fire and forget - CGEvents will queue)
        app.activate(options: [.activateIgnoringOtherApps])

        // 2. Set clipboard immediately
        let pb = NSPasteboard.general
        pb.clearContents()
        guard pb.setString(text, forType: .string) else {
            log.error("Failed to copy text to clipboard")
        ...
        // 3. Paste - CGEvents are synchronous, no delays needed
        let pasteResult = simulatePaste()

        log.info("📋 Clipboard paste completed: \(pasteResult)")
        return pasteResult
    }
    ...
    func insertAndSubmit(_ text: String, intoAppWithBundleID bundleID: String?) async -> Bool {
        ...
        // Press Enter to submit - CGEvents are synchronous
    ...
    func simulateEnterInApp(bundleId: String) async -> Bool {
        ...
        // Press Enter - CGEvents are synchronous
        let result = simulateEnter()
