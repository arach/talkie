...
final class ParticleTuning: ObservableObject {
    ...
    @Published var baseAmplitude: Double = 0.30 {
        didSet { saveCurrentSettings(); log("baseAmplitude", baseAmplitude) }
    }
    @Published var audioAmplitude: Double = 0.85 {
    ...
    @Published var smoothingFactor: Double = 0.55 {
...
struct StatusBar: View {
            ...
            ZStack {
                ...
                HStack(spacing: 8) {
                    LivePill(
                        ...
                        pendingQueueCount: UnifiedDatabase.countNeedsRetry(),
    ...
    private var regularRightContent: some View {
        ...
        case .refining:
            // Show milestone trail during refining too
            HStack(spacing: 4) {
                ForEach(milestones.milestones.suffix(3)) { milestone in
                    Image(systemName: milestone.icon)
                        .font(.system(size: 8))
    ...
    private var engineStatusIndicator: some View {
        ...
        .buttonStyle(.plain)
        .help("Click to open TalkieAgent")
    }
    ...
    private var engineStatusColor: Color {
        ...
        case .connectedWrongBuild:
            return .yellow
        case .connecting, .launchingEngine:
            return .orange
        case .disconnected, .error, .engineNotFound:
            return .red
        }
    }
...
struct EngineStatusIcon: View {
    ...
    private var statusColor: Color {
        ...
        case .connecting, .launchingEngine: return SemanticColor.info
        case .disconnected, .error, .engineNotFound: return SemanticColor.error
        }
    }
    ...
    private var label: String {
        ...
        case .launchingEngine: return "Starting"
        case .disconnected, .error: return "Offline"
        case .engineNotFound: return "Not Found"
        }
    }
    ...
    var body: some View {
        HStack(spacing: 6) {
            StatusIcon(
                ...
                color: statusColor,
                label: label,
                detail: "Embedded Engine - Click: Settings, Shift+Click: Reconnect",
                action: handleClick
            )
...
struct DebugToolbarOverlay<Content: View>: View {
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: 10) {
                ...
                DebugSection(title: "STATE") {
                    VStack(spacing: 0) {
                        debugInfoRow("Echoes", "\(UnifiedDatabase.countDictations())")
                ...
                DebugSection(title: "SYSTEM") {
                    VStack(spacing: 4) {
                        ...
                        DebugActionButton(icon: "trash", label: "Prune Old Data") {
                            ...
                            UnifiedDatabase.pruneDictations(olderThanHours: hours)
                ...
                DebugSection(title: "DATABASE") {
                    VStack(spacing: 4) {
                        HStack {
                            Text("\(UnifiedDatabase.countDictations()) records")
                                .font(.system(size: 10, weight: .medium, design: .monospaced))
                                .foregroundColor(.white.opacity(0.7))
                            Spacer()
                        }
                        ...
                        DebugActionButton(icon: "folder", label: "Reveal Database") {
                            NSWorkspace.shared.selectFile(
                                UnifiedDatabase.databaseURL.path,
                                inFileViewerRootedAtPath: UnifiedDatabase.databaseURL.deletingLastPathComponent().path
                            )
                        }
                    }
                }
    ...
    private func livePillPreviewRow<RowContent: View>(_ label: String, @ViewBuilder content: () -> RowContent) -> some View {
        HStack(spacing: 8) {
            Text(label)
                .font(.system(size: 9, weight: .medium))
                .foregroundColor(.white.opacity(0.5))
    ...
    private func copyDebugInfo() {
        ...
        lines.append("TalkieAgent \(appVersion) (\(buildNumber))")
        lines.append("macOS \(ProcessInfo.processInfo.operatingSystemVersionString)")
        lines.append("")

        // State
        lines.append("State:")
        lines.append("  Echoes: \(UnifiedDatabase.countDictations())")
    ...
    private func testInterstitialFlow() {
        ...
        // Create LiveDictation first (for conversion)
...
struct ObjectInspectorView: View {
    ...
    private let recordings = UnifiedDatabase.all()
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            HStack {
                ...
                if !recordings.isEmpty {
                    Picker("", selection: $selectedIndex) {
                        ForEach(Array(recordings.prefix(20).enumerated()), id: \.offset) { index, r in
                            Text("\(r.id.uuidString.prefix(8))... – \(r.text.prefix(20))...").tag(index)
                        }
                    }
                    .pickerStyle(.menu)
                    .frame(width: 200)
            ...
            if recordings.isEmpty {
                ...
                let r = recordings[min(selectedIndex, recordings.count - 1)]

                ScrollView {
                    VStack(alignment: .leading, spacing: 0) {
                        // Schema columns section
                        inspectorSection("DATABASE COLUMNS") {
                            inspectorRow("id", r.id.uuidString)
                            inspectorRow("createdAt", dateFormatter.string(from: r.createdAt))
                            inspectorRow("text", String(r.text.prefix(100)) + (r.text.count > 100 ? "..." : ""))
                            inspectorRow("duration", String(format: "%.2f", r.duration))
                            inspectorRow("transcriptionModel", r.transcriptionModel)
                            inspectorRow("audioFilename", r.audioFilename)
                            inspectorRow("transcriptionStatus", r.transcriptionStatus)
                            inspectorRow("transcriptionError", r.transcriptionError)
                        }

                        // Computed properties
                        inspectorSection("COMPUTED") {
                            inspectorRow("hasAudio", r.hasAudio ? "true" : "false")
                        }
                    }
                    .padding()
                }
