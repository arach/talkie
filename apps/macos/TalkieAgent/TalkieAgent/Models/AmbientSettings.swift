...
final class AmbientSettings: ObservableObject {
    ...
    private init() {
        ...
        self.isEnabled = store.bool(forKey: AgentSettingsKey.ambientEnabled)

        // Load wake phrase
        if let phrase = store.string(forKey: AgentSettingsKey.ambientWakePhrase), !phrase.isEmpty {
        ...
        // Load end phrase
        if let phrase = store.string(forKey: AgentSettingsKey.ambientEndPhrase), !phrase.isEmpty {
        ...
        // Load cancel phrase
        if let phrase = store.string(forKey: AgentSettingsKey.ambientCancelPhrase), !phrase.isEmpty {
        ...
        let duration = store.double(forKey: AgentSettingsKey.ambientBufferDuration)
        self.bufferDuration = duration > 0 ? duration : Self.defaultBufferDuration

        // Load chimes enabled (default: true)
        if store.object(forKey: AgentSettingsKey.ambientEnableChimes) != nil {
            self.enableChimes = store.bool(forKey: AgentSettingsKey.ambientEnableChimes)
        } else {
            self.enableChimes = true
        }

        // Load streaming ASR enabled (default: true - streaming is faster for wake detection)
        if store.object(forKey: AgentSettingsKey.ambientUseStreamingASR) != nil {
            self.useStreamingASR = store.bool(forKey: AgentSettingsKey.ambientUseStreamingASR)
        } else {
        ...
        if store.object(forKey: AgentSettingsKey.ambientUseBatchASR) != nil {
            self.useBatchASR = store.bool(forKey: AgentSettingsKey.ambientUseBatchASR)
        } else {
    ...
    private func save() {
        ...
        store.set(isEnabled, forKey: AgentSettingsKey.ambientEnabled)
        store.set(wakePhrase, forKey: AgentSettingsKey.ambientWakePhrase)
        store.set(endPhrase, forKey: AgentSettingsKey.ambientEndPhrase)
        store.set(cancelPhrase, forKey: AgentSettingsKey.ambientCancelPhrase)
        store.set(bufferDuration, forKey: AgentSettingsKey.ambientBufferDuration)
        store.set(enableChimes, forKey: AgentSettingsKey.ambientEnableChimes)
        store.set(useStreamingASR, forKey: AgentSettingsKey.ambientUseStreamingASR)
        store.set(useBatchASR, forKey: AgentSettingsKey.ambientUseBatchASR)
    }
