...
final class AmbientController: ObservableObject {
    ...
    private func processCommand(_ text: String) {
        ...
        // Check for voice navigation intent first
        Task { @MainActor in
            let wasNavigation = await VoiceNavigationHandler.shared.processCommand(text)

            if wasNavigation {
                // Navigation handled - update HUD and return to listening
                log.info("Command handled as navigation")
                AmbientHUDController.shared.addActivity(.system("Navigation: \(text)"))
            } else {
                // Not a navigation command - send as ambient command
                // Notify listener (callback)
                onCommandCaptured?(command)

                // Send to Talkie via XPC
                let bufferContext = getRecentTranscript(seconds: 60)
                TalkieAgentXPCService.shared.notifyAmbientCommand(
                    text,
