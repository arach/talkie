...
struct StorageSettingsSection: View {
            ...
            SettingsCard(title: "DATA MANAGEMENT") {
                VStack(alignment: .leading, spacing: Spacing.md) {
                    ...
                    HStack {
                        ...
                        Button("Open") {
                            if let url = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first?.appendingPathComponent("TalkieAgent") {
                    ...
                    HStack(spacing: Spacing.sm) {
                        StorageActionButton(
                            ...
                            UnifiedDatabase.pruneDictations(olderThanHours: settings.dictationTTLHours)
                            refreshStats()
                        }
    ...
    private func cleanOrphanedAudio() {
        let recordings = UnifiedDatabase.all()
        let referencedFilenames = Set(recordings.compactMap { $0.audioFilename })
...
struct StorageStats {
    ...
    static func calculate() async -> StorageStats {
        ...
        stats.totalUtterances = recordings.count
        // Compute word count from text
        stats.totalWords = recordings.reduce(0) { total, recording in
            total + recording.text.split(separator: " ").count
        }
        stats.totalDurationSeconds = recordings.map { $0.duration }.reduce(0, +)
        stats.audioStorageBytes = await AudioStorage.totalStorageBytesAsync()

        // Database size - now using unified database path
        if let dbPath = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first?
            .appendingPathComponent("Talkie/talkie_grdb.sqlite").path {
        ...
        // Top apps - extract from parsedMetadata
        var appCounts: [String: (name: String, count: Int)] = [:]
        for recording in recordings {
            let metadata = recording.parsedMetadata
            if let bundleID = metadata.app?.bundleId {
                let name = metadata.app?.name ?? bundleID
