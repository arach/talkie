...
final class QueuePickerController {
    ...
    func show() {
        ...
        let items = UnifiedDatabase.fetchQueued()
    ...
    private func pasteAndDismiss(_ item: LiveRecording) {
        let text = item.text

        // Mark as pasted in database
        UnifiedDatabase.markPasted(id: item.id)
...
final class QueuePickerViewModel: ObservableObject {
    @Published var items: [LiveRecording]
    @Published var selectedIndex: Int = 0

    var onSelect: ((LiveRecording) -> Void)?
    var onDismiss: (() -> Void)?

    var selectedItem: LiveRecording? {
        guard selectedIndex >= 0, selectedIndex < items.count else { return nil }
        return items[selectedIndex]
    }

    init(items: [LiveRecording]) {
        self.items = items
    }
    ...
    func select(_ item: LiveRecording) {
        onSelect?(item)
    }
...
#Preview {
    let items = [
        LiveRecording(text: "This is a test utterance that was queued because it was recorded inside Talkie Agent"),
        LiveRecording(text: "Another queued item with some longer text to show how it wraps and truncates properly"),
        LiveRecording(text: "Short one"),
    ]
    let viewModel = QueuePickerViewModel(items: items)
