...
final class FailedQueueController {
    ...
    func show() {
        ...
        let items = UnifiedDatabase.fetchNeedsRetry()
...
final class FailedQueueViewModel: ObservableObject {
    ...
    @Published var retryingItems: Set<UUID> = []
    ...
    func retry(_ item: LiveRecording) {
        let id = item.id
    ...
    func delete(_ item: LiveRecording) {
        let id = item.id

        // Delete from database (this also removes the audio file)
        UnifiedDatabase.delete(id: id)
            ...
            ScrollView(.vertical, showsIndicators: false) {
                VStack(spacing: 1) {
                    ForEach(Array(viewModel.items.enumerated()), id: \.element.id) { index, item in
                        FailedItemRow(
                            item: item,
                            isRetrying: viewModel.retryingItems.contains(item.id),
...
#Preview {
    FailedQueueView(viewModel: FailedQueueViewModel(items: [
        LiveRecording(
            text: "",
            duration: 12.5,
