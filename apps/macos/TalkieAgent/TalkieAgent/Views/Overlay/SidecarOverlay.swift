...
final class SidecarOverlayController: ObservableObject {
    ...
    /// Check if any Sidecar display is connected (use before calling shared instance)
    static var isSidecarConnected: Bool {
        NSScreen.screens.contains { $0.isSidecar && $0.isValid }
    }
    ...
    func updateState(_ newState: LiveState) {
        // Skip if no Sidecar connected and overlay not already visible
        guard isVisible || Self.isSidecarConnected else { return }
...
struct SidecarOverlayView: View {
    ...
    private var recordingView: some View {
        VStack(spacing: 16) {
            ...
            HStack(spacing: 24) {
                SidecarActionButton(icon: "xmark", label: "Cancel", cornerRadius: cornerRadius - 4) {
                    controller.requestCancel()
                }

                SidecarActionButton(icon: "checkmark", label: "Done", cornerRadius: cornerRadius - 4) {
