...
```
TalkieAgent (menu bar)          TalkieEngine (XPC)         Talkie (main UI)
     │                                │                          │
...
- **Location**: `apps/macos/TalkieEngine/`
...
```swift
// TalkieAgent - real-time dictation (user is waiting)
...
**Why?**
- TalkieAgent can write dictations when Talkie isn't running
...
1. TalkieAgent detects voice → starts recording
2. User stops speaking → audio saved to temp file
3. TalkieAgent → `engine.transcribe(path, model, priority: .high, ...)`
4. Engine transcribes (high priority, not starved by builds)
5. TalkieAgent saves to `live.sqlite` with transcript
...
**Example**: `TalkieAgentStateMonitor.utteranceWasAdded()` → `DictationStore.refresh()`
...
### Architecture
- `apps/macos/SYSTEM_ARCHITECTURE.md` (this file) - System overview
- `apps/macos/AGENTS.md` - Build commands, project structure
- `apps/macos/Talkie/ARCHITECTURE_REVIEW.md` - SwiftUI performance patterns

### Startup & Performance
- `apps/macos/Talkie/App/StartupCoordinator.swift` - Phased initialization
- `apps/macos/Talkie/App/AppDelegate.swift` - Main app lifecycle
- `apps/macos/Talkie/Stores/DictationStore.swift` - Lazy loading, polling

### Transcription Priority
- `apps/macos/TalkieEngine/EngineProtocol.swift` - XPC protocol, priority enum
- `apps/macos/TalkieEngine/EngineService.swift` - Priority → Task priority conversion

### Database
- `apps/macos/Talkie/Database/LiveDatabase.swift` - Shared GRDB interface
- `apps/macos/TalkieAgent/Database/LiveDatabase.swift` - Mirror (same file)

### XPC Communication
- `apps/macos/Talkie/Services/TalkieAgentStateMonitor.swift` - Receives callbacks
- `apps/macos/TalkieAgent/Services/TalkieAgentXPCService.swift` - Broadcasts updates
...
→ Check XPC connection: `TalkieAgentStateMonitor.shared.isXPCConnected`
