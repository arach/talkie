...
struct DictationMetadata: Codable, Hashable {
    ...
    var perfInAppMs: Int?         // TalkieAgent processing (endToEnd - engine)
...
struct Dictation: Identifiable, Codable, Hashable {
    ...
    /// Database ID (UUID string from UnifiedDatabase) - used for linking to database records
    var liveID: String?
    ...
    /// Initialize from database record — uses TalkieObject UUID as identity so navigation works
    init(text: String, durationSeconds: Double?, metadata: DictationMetadata, timestamp: Date, liveID: String?) {
        // Preserve the TalkieObject's UUID so selecting a dictation from the home page
        // can highlight the matching row in RecordingsScreen
        self.id = liveID.flatMap({ UUID(uuidString: $0) }) ?? UUID()
        self.text = text
...
struct ContextCapture {
    ...
    static let talkieAgentBundleID = Bundle.main.bundleIdentifier ?? "live.talkie.TalkieAgent"

    /// Check if Talkie Agent is the frontmost app
    static func isTalkieAgentFrontmost() -> Bool {
        guard let frontApp = NSWorkspace.shared.frontmostApplication else { return false }
        return frontApp.bundleIdentifier == talkieAgentBundleID
    }
...
final class DictationStore {
    ...
    private static let initialLoadSize = 8
    ...
    /// High water mark: most recent timestamp we've seen (for incremental sync)
    private var lastSeenDate: Date?

    /// Repository for database access
    private let recordingRepo = TalkieObjectRepository()

    /// Current refresh task - tracked to prevent race conditions
    private var refreshTask: Task<Void, Never>?
    /// Number of new dictations added by the most recent refresh pass.
    private(set) var lastRefreshAddedCount = 0
    ...
    func add(_ text: String, durationSeconds: Double? = nil, metadata: DictationMetadata = DictationMetadata()) {
        ...
        logger.warning("store not implemented - should route through TalkieAgent XPC")
    ...
    /// TODO: Route through TalkieAgent XPC instead of direct DB write
    func updateText(for id: UUID, newText: String, modelId: String? = nil) {
        logger.warning("updateText not implemented - should route through TalkieAgent XPC")
    }

    /// Delete a dictation
    /// TODO: Route through TalkieAgent XPC instead of direct DB write
    func delete(_ dictation: Dictation) {
        logger.warning("delete not implemented - should route through TalkieAgent XPC")
    ...
    /// TODO: Route through TalkieAgent XPC instead of direct DB write
    func clear() {
        logger.warning("clear not implemented - should route through TalkieAgent XPC")
    ...
    /// TODO: Route through TalkieAgent XPC - Talkie is read-only
    func pruneExpired() {
        logger.warning("prune not implemented - should route through TalkieAgent XPC")
        refresh()
    }
    ...
    func refresh() -> Task<Void, Never> {
        ...
        refreshTask = Task {
            do {
                ...
                // Incremental fetch: only get new dictations since lastSeenDate
                if let since = lastSeenDate {
                    recordings = try await recordingRepo.fetchDictationsSince(date: since)
                    ...
                    logger.info("Incremental refresh: found \(recordings.count) new dictations since \(since)")
                } else {
                    // First load: only get recent dictations for fast initial render
                    recordings = try await recordingRepo.fetchRecentDictations(limit: Self.initialLoadSize)
                    logger.debug("Initial load: loaded \(recordings.count) recent dictations (limit: \(Self.initialLoadSize))")
                ...
                // Update high water mark to most recent timestamp seen
                if let maxDate = recordings.map(\.createdAt).max() {
                    await MainActor.run {
                        self.lastSeenDate = maxDate
                    }
                }

                // Build map of existing dictations by liveID to preserve UUIDs and selection state
                var existingByLiveID: [String: Dictation] = [:]
                ...
                // Convert new Recordings to Dictations
                let newUtterances = recordings.compactMap { recording -> Dictation? in
                    let liveIDString = recording.id.uuidString
                    if let existing = existingByLiveID[liveIDString] {
                        // Check if data has changed
                        let newMetadata = buildMetadata(from: recording)
                        if existing.text == recording.text &&
                           existing.durationSeconds == recording.duration &&
                           existing.metadata == newMetadata &&
                           existing.timestamp == recording.createdAt {
                            // No changes - reuse existing instance to preserve UUID and selection
                            return existing
                        }
                        // Data changed - update in place
                        var updated = existing
                        updated.text = recording.text ?? ""
                    ...
                    return Dictation(
                        ...
                        metadata: buildMetadata(from: recording),
                        timestamp: recording.createdAt,
                        liveID: recording.id.uuidString
                    )
                }

                let insertedCount = recordings.reduce(into: 0) { partial, recording in
                    if existingByLiveID[recording.id.uuidString] == nil {
                        partial += 1
                    }
                }
                ...
                await MainActor.run {
                    ...
                    if self.lastSeenDate != nil && !newUtterances.isEmpty {
                        ...
                        let remainingExisting = self.dictations.filter { dictation in
                            guard let liveID = dictation.liveID else { return true }
                            return !fetchedLiveIDs.contains(liveID)
                        }

                        // Merge and sort
                        self.dictations = (newUtterances + remainingExisting).sorted { $0.timestamp > $1.timestamp }
                        self.lastRefreshAddedCount = insertedCount
                        logger.info("Added \(newUtterances.count) new dictations (total: \(self.dictations.count))")
                    } else if isFirstLoad {
                        // Full refresh: replace entire array
                        self.dictations = newUtterances
                        self.lastRefreshAddedCount = insertedCount
                ...
                // Task was cancelled - this is expected when refresh() is called rapidly
                await MainActor.run {
                    self.lastRefreshAddedCount = 0
                }
                logger.debug("Refresh task cancelled (superseded by newer refresh)")
            } catch {
                await MainActor.run {
                    self.lastRefreshAddedCount = 0
                }
                logger.error("Failed to refresh dictations: \(error)")
            }
        }
    ...
    /// Refresh and await completion. Returns number of newly added dictations.
    func refreshAndWait() async -> Int {
        let task = refresh()
        await task.value
        return lastRefreshAddedCount
    }
    ...
    /// Build DictationMetadata from TalkieObject
    private func buildMetadata(from recording: TalkieObject) -> DictationMetadata {
        // Parse the JSON metadata if available
        let recordingMeta = RecordingMetadata.from(json: recording.metadataJSON)

        // Build metadata in parts to help type checker
        var metadata = DictationMetadata()

        // App context
        metadata.activeAppBundleID = recordingMeta?.app?.bundleId
        metadata.activeAppName = recordingMeta?.app?.name
        metadata.activeWindowTitle = recordingMeta?.app?.windowTitle
        metadata.endAppBundleID = recordingMeta?.endApp?.bundleId
        metadata.endAppName = recordingMeta?.endApp?.name
        metadata.endWindowTitle = recordingMeta?.endApp?.windowTitle

        // Rich context
        metadata.documentURL = recordingMeta?.context?.documentURL
        metadata.browserURL = recordingMeta?.context?.browserURL
        metadata.terminalWorkingDir = recordingMeta?.context?.terminalWorkingDir

        // Transcription
        metadata.transcriptionModel = recording.transcriptionModel

        // Performance metrics
        metadata.perfEngineMs = recordingMeta?.performance?.engineMs
        metadata.perfEndToEndMs = recordingMeta?.performance?.endToEndMs
        metadata.perfInAppMs = recordingMeta?.performance?.inAppMs
        metadata.sessionID = recordingMeta?.performance?.sessionId

        // Audio metrics
        metadata.peakAmplitude = recordingMeta?.audio?.peakAmplitude
        metadata.averageAmplitude = recordingMeta?.audio?.averageAmplitude
        metadata.audioFilename = recording.audioFilename

        // Routing info
        metadata.routingMode = recordingMeta?.routing?.mode
        metadata.wasRouted = recordingMeta?.routing?.wasRouted ?? false

        return metadata
    }

    /// Search dictations
    func search(_ query: String) async -> [Dictation] {
        do {
            let results = try await recordingRepo.searchDictations(query: query)
            return results.map { recording in
                Dictation(
                    text: recording.text ?? "",
                    durationSeconds: recording.duration,
                    metadata: buildMetadata(from: recording),
            ...
            logger.error("Failed to search dictations: \(error)")
            return []
        }
    }
