...
final class ExternalDataAuditor {
    ...
    private func getCoreDataExternalReferences() async throws -> Set<String> {
        // NOTE: Core Data now lives in TalkieSync - we can't query it directly
        // For audio references, we rely on GRDB which has the audioFilePath
        // Return empty set - audio audit should use getGRDBReferences() instead
        return []
    }
