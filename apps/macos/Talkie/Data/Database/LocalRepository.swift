...
actor LocalRepository: MemoRepository {
    nonisolated private let dbManager: DatabaseManager
        ...
        try await instrumentRepositoryRead("fetchMemo") {
            ...
            return try await db.read { db in
                ...
                guard let memo = try MemoModel
                    .filter(MemoModel.Columns.id == id)
                    .fetchOne(db) else {
                    return nil
                }
                ...
                let transcripts = try TranscriptVersionModel
                    .filter(TranscriptVersionModel.Columns.memoId == id)
                    .order(TranscriptVersionModel.Columns.version.desc)
                    .fetchAll(db)

                let workflows = try WorkflowRunModel
                    .filter(WorkflowRunModel.Columns.memoId == id)
    ...
    func deleteMemo(id: UUID) async throws {
        try await instrumentRepositoryWrite("deleteMemo") {
            ...
            let deleted = try await db.write { db in
                // Use GRDB's proper delete-by-key method (same encoding as fetchOne)
                try MemoModel.deleteOne(db, key: id)
            }

            if deleted {
                log.info("🗑️ Delete id=\(id.uuidString.prefix(8)): removed successfully")
            } else {
                log.warning("🗑️ Delete id=\(id.uuidString.prefix(8)): memo not found in database")
            }
        }
    }
        ...
        try await instrumentRepositoryRead("fetchTranscriptVersions") {
            ...
            return try await db.read { db in
                try TranscriptVersionModel
                    .filter(TranscriptVersionModel.Columns.memoId == memoId)
                    .order(TranscriptVersionModel.Columns.version.desc)
                    .fetchAll(db)
            }
        }
    ...
    func deleteWorkflowRun(id: UUID) async throws {
        try await instrumentRepositoryWrite("deleteWorkflowRun") {
            ...
            _ = try await db.write { db in
                try WorkflowRunModel
                    .filter(WorkflowRunModel.Columns.id == id)
                    .deleteAll(db)
            }
        }
    }
