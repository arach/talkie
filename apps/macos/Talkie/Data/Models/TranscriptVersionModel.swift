...
struct TranscriptVersionModel: Identifiable, Codable, Hashable {
    let id: UUID
    let memoId: UUID
    var version: Int
    var content: String
...
extension TranscriptVersionModel: FetchableRecord, PersistableRecord {
...
extension TranscriptVersionModel {
    var sourceTypeEnum: TranscriptVersionModel.SourceType? {
        TranscriptVersionModel.SourceType(rawValue: sourceType)
    }

    var engineDisplayName: String {
        TranscriptVersionModel.Engines.displayName(for: engine)
    }
...
extension TranscriptVersionModel: Sendable {}
