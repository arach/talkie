...
class WhisperService {
    ...
    func transcribe(
        ...
        guard connected else {
            lastError = "Embedded engine not available"
            throw WhisperError.engineNotAvailable
        }

        logger.info("[Engine] Transcribing \(audioData.count / 1024)KB via embedded engine (priority: \(priority.displayName))")
        await SystemEventManager.shared.log(.transcribe, "Using embedded engine", detail: model.displayName)
...
enum WhisperError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .engineNotAvailable:
            return "The embedded engine is not available"
        }
    }
}
