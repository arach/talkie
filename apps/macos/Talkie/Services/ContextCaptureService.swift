...
final class ContextCaptureService {
    ...
    /// TODO: Route metadata updates through TalkieAgent XPC instead of direct DB write
    func scheduleEnrichment(utteranceId: Int64, baseline: DictationMetadata) {
        // Disabled - metadata enrichment should be done by TalkieAgent
        // which owns the write path to LiveDatabase
        logger.debug("scheduleEnrichment disabled - should route through TalkieAgent XPC")
    }
