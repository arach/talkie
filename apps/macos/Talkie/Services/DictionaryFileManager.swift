...
actor DictionaryFileManager {
    ...
    // MARK: - System Dictionary Migration

    /// Ensure one dictionary has source `.system` (the default dictionary).
    /// If none exists, promote the "Personal" dictionary or the first `.manual` one.
    func migrateToSystemDefault() async throws {
        // Already have a system dictionary — nothing to do
        if manifest.dictionaries.contains(where: { $0.source == .system }) {
            return
        }

        // Find candidate: prefer one named "Personal", otherwise first .manual
        let candidateIndex = manifest.dictionaries.firstIndex(where: { $0.name == "Personal" && $0.source == .manual })
            ?? manifest.dictionaries.firstIndex(where: { $0.source == .manual })

        guard let index = candidateIndex else { return }

        let entry = manifest.dictionaries[index]

        // Load full dictionary, update source, save back
        var dict = try await loadDictionary(id: entry.id)
        dict.source = .system
        try await saveDictionary(dict)

        log.info("Migrated dictionary to system default", detail: entry.name)
    }
