...
final class APIKeyStore {
    ...
    private func syncAllToSharedSettings() {
        for provider in Provider.allCases {
            if let value = cache[provider.rawValue], !value.isEmpty {
                if let key = sharedSettingsKey(for: provider) {
    ...
    func set(_ value: String?, for provider: Provider) {
        ...
        save()
        syncToSharedSettings(provider: provider, value: value)
    }

    /// Sync API key to TalkieSharedSettings for TalkieAgent access
    private func syncToSharedSettings(provider: Provider, value: String?) {
        guard let key = sharedSettingsKey(for: provider) else { return }

        if let value = value, !value.isEmpty {
            TalkieSharedSettings.set(value, forKey: key)
            logger.debug("Synced \(provider.displayName) API key to shared settings")
        } else {
            TalkieSharedSettings.removeObject(forKey: key)
            logger.debug("Removed \(provider.displayName) API key from shared settings")
        }
    }

    /// Map provider to shared settings key
    private func sharedSettingsKey(for provider: Provider) -> String? {
        switch provider {
        ...
        case .elevenLabs: return AgentSettingsKey.elevenLabsApiKey
        }
    }
