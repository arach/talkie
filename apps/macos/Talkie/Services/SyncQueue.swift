...
actor SyncQueue {
    ...
    private func processItem(_ item: PendingItem) async {
        ...
        do {
            ...
            if case let SyncClientError.rateLimited(until) = error {
                mutableItem.nextRetryAt = until
                pending[item.id] = mutableItem
                let remaining = max(0, Int(until.timeIntervalSinceNow.rounded()))
                log.info("[\(item.shortId)] Deferred by sync rate limit (\(remaining)s remaining)")
                return
            }
    ...
    private func triggerCloudKitExport() async throws {
        // NOTE: Core Data removed from main app - CloudKit export handled by TalkieSync
        // Trigger sync via SyncClient instead
