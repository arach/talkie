...
final class DictionaryManager: ObservableObject {
    ...
    /// Written to shared settings so TalkieAgent can read it
    @Published var isGloballyEnabled: Bool {
        didSet {
            TalkieSharedSettings.set(isGloballyEnabled, forKey: AgentSettingsKey.dictionaryEnabled)
            syncToEngine()
        }
    }

    /// Enable/disable symbolic mapping (slash → /, dash → -, etc.)
    /// Written to shared settings so TalkieAgent can read it
    @Published var isSymbolicMappingEnabled: Bool {
        didSet {
            TalkieSharedSettings.set(isSymbolicMappingEnabled, forKey: AgentSettingsKey.symbolicMappingEnabled)
            Task {
                await EngineClient.shared.setSymbolicMappingEnabled(isSymbolicMappingEnabled)
            }
        }
    }

    /// Enable/disable filler-word removal (um/uh/uhm variants)
    /// Written to shared settings so TalkieAgent can read it
    @Published var isFillerRemovalEnabled: Bool {
        didSet {
            TalkieSharedSettings.set(isFillerRemovalEnabled, forKey: AgentSettingsKey.fillerRemovalEnabled)
            Task {
                await EngineClient.shared.setFillerRemovalEnabled(isFillerRemovalEnabled)
            }
        }
    }
    ...
    /// The default (system) dictionary — always present, not deletable
    var defaultDictionary: TalkieDictionary? {
        dictionaries.first(where: { $0.source == .system })
    }

    /// All non-system dictionaries (named/additional)
    var namedDictionaries: [TalkieDictionary] {
        dictionaries.filter { $0.source != .system }
    }
    ...
    private init() {
        self.isGloballyEnabled = TalkieSharedSettings.bool(forKey: AgentSettingsKey.dictionaryEnabled)
        // Default to true if not set (matches previous always-on behavior)
        if TalkieSharedSettings.object(forKey: AgentSettingsKey.symbolicMappingEnabled) == nil {
            self.isSymbolicMappingEnabled = true
        } else {
            self.isSymbolicMappingEnabled = TalkieSharedSettings.bool(forKey: AgentSettingsKey.symbolicMappingEnabled)
        }
        if TalkieSharedSettings.object(forKey: AgentSettingsKey.fillerRemovalEnabled) == nil {
            self.isFillerRemovalEnabled = true
        } else {
            self.isFillerRemovalEnabled = TalkieSharedSettings.bool(forKey: AgentSettingsKey.fillerRemovalEnabled)
        }
    }
    ...
    func load() async {
        ...
        do {
            ...
            // Migrate default dictionary to .system source
            try await DictionaryFileManager.shared.migrateToSystemDefault()
    ...
    /// Create a default system dictionary if none exists
    func ensureDefaultDictionary() async {
        guard defaultDictionary == nil else { return }
        do {
            let dictionary = try await DictionaryFileManager.shared.createDictionary(
                name: "Personal",
                description: "Your personal word replacements",
                source: .system
            )
            dictionaries.insert(dictionary, at: 0)
            log.info("Default system dictionary created")
        } catch {
            log.error("Failed to create default dictionary", error: error)
        }
    }
}
