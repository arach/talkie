...
class SpeechSynthesisService: NSObject {
    ...
    private func applySavedVoicePreference() {
        let savedVoiceId = SettingsManager.shared.selectedTTSVoiceId
        if savedVoiceId.hasPrefix("com.apple.voice"),
           AVSpeechSynthesisVoice(identifier: savedVoiceId) != nil {
            selectedVoiceIdentifier = savedVoiceId
            logger.info("Selected saved Apple voice: \(savedVoiceId)")
            return
        }
...
// MARK: - Cloud TTS Service (direct HTTP, no external dependencies)
...
enum TTSService {
    ...
    static func synthesizeElevenLabs(text: String, voiceId: String, apiKey: String?) async throws -> URL {
        guard let apiKey, !apiKey.isEmpty else {
            throw CloudTTSError.missingElevenLabsKey
        }
        ...
        guard let requestURL = components?.url else {
            throw CloudTTSError.requestFailed("Could not build the ElevenLabs request URL.")
        }
        ...
        request.httpBody = try JSONEncoder().encode(Body(text: text, model_id: "eleven_flash_v2_5"))

        let (data, response) = try await URLSession.shared.data(for: request)
        guard let http = response as? HTTPURLResponse, (200..<300).contains(http.statusCode) else {
            let message = String(data: data, encoding: .utf8)?.trimmingCharacters(in: .whitespacesAndNewlines)
            let code = (response as? HTTPURLResponse)?.statusCode ?? 0
            throw CloudTTSError.requestFailed(message?.isEmpty == false ? message! : "ElevenLabs request failed (\(code)).")
        }
    ...
    static func synthesizeOpenAI(text: String, voice: String, apiKey: String?) async throws -> URL {
        guard let apiKey, !apiKey.isEmpty else {
            throw CloudTTSError.missingOpenAIKey
        }
        ...
        struct Body: Encodable { let model: String; let input: String; let voice: String; let response_format: String }

        var request = URLRequest(url: URL(string: "https://api.openai.com/v1/audio/speech")!)
        request.httpMethod = "POST"
        request.setValue("application/json", forHTTPHeaderField: "Content-Type")
        request.setValue("Bearer \(apiKey)", forHTTPHeaderField: "Authorization")
        request.httpBody = try JSONEncoder().encode(Body(model: "tts-1", input: text, voice: voice, response_format: "mp3"))

        let (data, response) = try await URLSession.shared.data(for: request)
        guard let http = response as? HTTPURLResponse, (200..<300).contains(http.statusCode) else {
            let message = String(data: data, encoding: .utf8)?.trimmingCharacters(in: .whitespacesAndNewlines)
            let code = (response as? HTTPURLResponse)?.statusCode ?? 0
            throw CloudTTSError.requestFailed(message?.isEmpty == false ? message! : "OpenAI TTS request failed (\(code)).")
        }
