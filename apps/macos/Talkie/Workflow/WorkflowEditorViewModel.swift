...
final class WorkflowEditorViewModel {
    ...
    /// Move a step by indices (convenience for non-IndexSet calls)
    func moveStep(from source: Int, to destination: Int) {
        guard var draft = editingDraft else { return }
        guard source >= 0 && source < draft.steps.count else { return }
        var steps = draft.steps
    ...
    /// Add a step by type (creates default config)
    func addStep(ofType type: WorkflowStep.StepType) {
        guard let draft = editingDraft else { return }
        let newStep = WorkflowStep(
            type: type,
            config: StepConfig.defaultConfig(for: type),
            outputKey: "output_\(draft.steps.count + 1)"
        )
        addStep(newStep)
        log.info("Added step of type \(type.rawValue), total steps: \(draft.steps.count + 1)")
    }

    /// Update a step at a specific index
    func updateStep(at index: Int, with step: WorkflowStep) {
        guard var draft = editingDraft else { return }
        guard index >= 0 && index < draft.steps.count else { return }
        var steps = draft.steps
        steps[index] = step
        draft = WorkflowDefinition(
            id: draft.id,
    ...
    /// Get a binding to a step at a specific index (for step editors)
    func stepBinding(at index: Int) -> Binding<WorkflowStep> {
        Binding(
            get: {
                guard let draft = self.editingDraft,
                      index >= 0 && index < draft.steps.count else {
                    // Return a dummy step if out of bounds
                    return WorkflowStep(
                        type: .llm,
                        config: .llm(LLMStepConfig(provider: .openai, prompt: "")),
                        outputKey: ""
                    )
                }
            ...
            set: { newStep in
                self.updateStep(at: index, with: newStep)
            }
        )
    }

    /// Start editing a specific workflow by ID
    func startEditing(workflowID: UUID) {
        guard let workflow = workflowService.workflow(byID: workflowID) else { return }
        selectedWorkflowID = workflowID
        editingDraft = workflow.definition
    }

    /// Start editing a new workflow from a definition (for templates)
    func startEditing(newWorkflow: WorkflowDefinition) {
        selectedWorkflowID = newWorkflow.id
        editingDraft = newWorkflow
    }
    ...
    /// The steps array for display (from draft if editing, otherwise from selected workflow)
    var displayedSteps: [WorkflowStep] {
        displayedWorkflow?.steps ?? []
    }
}
