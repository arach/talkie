...
/// Identifies where a workflow came from (file storage location)
enum WorkflowFileSource: String, Codable {
    case system     // Protected, synced from bundle on update
    case user       // User-created, never touched by app
    case imported   // Imported via URL, stored with credentials
...
final class WorkflowFileRepository {
    ...
    private static func directoryURL(for source: WorkflowFileSource) -> URL {
    ...
    private func ensureDirectoriesExist() {
        ...
        for source in [WorkflowFileSource.system, .user, .imported] {
    ...
    func reloadAll() async {
        ...
        // Load system first, then user, then imported (later sources override earlier if same ID)
        for source in [WorkflowFileSource.system, .user, .imported] {
            let workflows = await loadWorkflows(from: source)
            all.append(contentsOf: workflows)
        }

        // Deduplicate by ID - keep the later one (user workflows override system)
        workflowsByID = Dictionary(all.map { ($0.id, $0) }) { existing, new in
            log.warning("Duplicate workflow ID: \(existing.id) - keeping '\(new.definition.name)' from \(new.source), discarding '\(existing.definition.name)' from \(existing.source)")
            return new
        }

        // Use deduplicated list (from dictionary values) to ensure no duplicates
        loadedWorkflows = Array(workflowsByID.values)
        log.info("Loaded \(loadedWorkflows.count) workflows")
    }

    private func loadWorkflows(from source: WorkflowFileSource) async -> [LoadedWorkflow] {
    ...
    func workflows(for source: WorkflowFileSource) -> [LoadedWorkflow] {
        loadedWorkflows.filter { $0.source == source }
    }

    /// Save a workflow to the specified source directory
    /// Handles renames by deleting the old file if the slug changes
    func save(_ workflow: WorkflowDefinition, slug: String? = nil, source: WorkflowFileSource = .user) async throws {
        guard source != .system else {
            throw WorkflowFileError.cannotDeleteSystemWorkflow
        }

        let effectiveSlug = slug ?? slugify(workflow.name)
        let newFileURL = Self.directoryURL(for: source).appendingPathComponent("\(effectiveSlug).json")

        // Check if this workflow already exists under a different filename (rename case)
        if let existingWorkflow = workflowsByID[workflow.id],
           existingWorkflow.source.isEditable,
           existingWorkflow.filePath != newFileURL {
            // Name changed - delete the old file to prevent duplicates
            let fm = FileManager.default
            if fm.fileExists(atPath: existingWorkflow.filePath.path) {
                try fm.removeItem(at: existingWorkflow.filePath)
                log.info("Renamed workflow: deleted old file \(existingWorkflow.filePath.lastPathComponent)")
            }
        }
        ...
        log.info("Saved workflow: \(workflow.name) to \(newFileURL.lastPathComponent)")
