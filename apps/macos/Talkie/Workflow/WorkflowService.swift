...
final class WorkflowService {
    ...
    private let configStore = WorkflowConfigurationStore.shared

    // MARK: - Initialization

    private init() {
        AppMode.guard(.lite, "WorkflowService")
    }
    ...
    func reload() async {
        ...
        let allPrefs = (try? configStore.synchronize(loadedWorkflows: loaded, using: prefsRepo))
            ?? (try? prefsRepo.fetch(for: loaded.map(\.id)))
            ?? [:]
        ...
        // Use uniquingKeysWith to handle any duplicate IDs gracefully (shouldn't happen, but safety first)
        workflowsByID = Dictionary(workflows.map { ($0.id, $0) }) { _, new in new }
        syncPinnedToiCloud()
    }
    ...
    /// - Parameter sourceAppBundleID: The bundle ID of the app where recording started (optional filtering)
    /// - Returns: Workflows configured to appear in interstitial, filtered by app context
    func actionsForInterstitial(sourceAppBundleID: String? = nil) -> [Workflow] {
        do {
            let actionIDs = try prefsRepo.fetchInterstitialActionIDs(sourceAppBundleID: sourceAppBundleID)
            return actionIDs.compactMap { workflowsByID[$0] }
        } catch {
            log.error("Failed to fetch interstitial actions: \(error)")
            return []
        }
    }

    /// Get actions for drafts/quick edit mode
    /// - Parameter appPreset: Optional app preset name to filter by (e.g., "Email", "Code")
    /// - Returns: Workflows configured to appear in drafts, filtered by app preset
    func actionsForDrafts(appPreset: String? = nil) -> [Workflow] {
        do {
            let actionIDs = try prefsRepo.fetchDraftsActionIDs(appPreset: appPreset)
            return actionIDs.compactMap { workflowsByID[$0] }
        } catch {
            log.error("Failed to fetch drafts actions: \(error)")
            return []
        }
    }

    /// Get all available app presets for the drafts preset picker
    /// - Returns: Sorted list of preset names (e.g., ["Code", "Email", "Slack"])
    func availableAppPresets() -> [String] {
        do {
            return try prefsRepo.fetchAppPresets()
        } catch {
            log.error("Failed to fetch app presets: \(error)")
            return []
        }
    }

    /// Check if a workflow is configured as an action (shows in any context)
    func isAction(_ workflow: Workflow) -> Bool {
        do {
            let pref = try prefsRepo.fetch(for: workflow.id)
            return (pref?.showInInterstitial ?? false) || (pref?.showInDrafts ?? false)
        } catch {
            return false
        }
    }

    /// Get all workflows configured as actions (for the Actions settings view)
    var allActions: [Workflow] {
        workflows.filter { isAction($0) }
    }

    /// Update action context for a workflow
    func setActionContext(
        for workflowId: UUID,
        showInInterstitial: Bool? = nil,
        showInDrafts: Bool? = nil,
        appBundleIDs: [String]? = nil
    ) async throws {
        let fallback = workflowsByID[workflowId]
            .map { WorkflowConfiguration.WorkflowPreferenceSnapshot(workflow: $0.definition) }
            ?? .init()

        configStore.updateWorkflowPreference(for: workflowId, defaultingTo: fallback) {
            if let showInInterstitial { $0.showInInterstitial = showInInterstitial }
            if let showInDrafts { $0.showInDrafts = showInDrafts }
            if let appBundleIDs { $0.appBundleIDs = appBundleIDs }
        }
        await reload()
    }
    ...
    func delete(_ workflow: Workflow) async throws {
        ...
        configStore.removeWorkflowPreference(for: workflow.id)
        await reload()
    }
