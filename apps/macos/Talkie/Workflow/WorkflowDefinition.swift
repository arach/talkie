...
struct WorkflowDefinition: Identifiable, Codable, Hashable {
    ...
    var source: WorkflowSource  // Where this workflow came from
    var createdAt: Date
    var modifiedAt: Date
    ...
    init(from decoder: Decoder) throws {
        ...
        source = try container.decodeIfPresent(WorkflowSource.self, forKey: .source) ?? .user
    ...
    private static var defaultGeminiModelId: String {
        LLMConfig.shared.defaultModel(for: "gemini") ?? "gemini-2.0-flash"
    }
    ...
    // MARK: - Speak Summary Workflow

    /// Summarize a memo dramatically, then speak the result using a remote TTS provider.
    static let speakSummary = WorkflowDefinition(
        id: UUID(uuidString: "00000000-7757-7E57-0000-000000000001")!,
        name: "Speak Summary",
        description: "Summarize memo as a movie trailer narrator, then speak it aloud with OpenAI TTS",
        icon: "speaker.wave.2",
        color: .orange,
        steps: [
            ...
            // Step 2: Speak it using OpenAI TTS
            WorkflowStep(
    ...
    static let defaultWorkflows = [summarize, extractTasks, keyInsights, brainDumpProcessor, speakSummary]
}
...
struct WorkflowStep: Identifiable, Codable {
    ...
    enum StepType: String, Codable, CaseIterable {
        ...
        var icon: String {
            ...
            case .cloudUpload: return "cloud.fill"
            }
        }
        ...
        var description: String {
            ...
            case .cloudUpload: return "Upload audio to cloud storage"
            }
        }
        ...
        var category: StepCategory {
            ...
            case .cloudUpload: return .integration
            }
        }
        ...
        var isAvailableInRunner: Bool {
            switch self {
            case .appleNotes, .appleCalendar:
                return false
            default:
                return true
            }
        ...
        /// Theme color for step type — neutral to match app-wide monochrome aesthetic
    ...
    enum StepCategory: String, CaseIterable {
        ...
        var steps: [StepType] {
            StepType.allCases.filter { $0.category == self && $0.isAvailableInRunner }
        }
    }
}
...
enum StepConfig: Codable {
    ...
    // Cloud storage
    case cloudUpload(CloudUploadStepConfig)
    ...
    static func defaultConfig(for type: WorkflowStep.StepType) -> StepConfig {
        ...
        case .cloudUpload:
            return .cloudUpload(CloudUploadStepConfig(provider: .r2, bucket: "", pathTemplate: "audio/{{MEMO_ID}}.m4a"))
        }
    }
}
...
enum WorkflowLLMProvider: String, Codable, CaseIterable {
    ...
    var models: [WorkflowModelOption] {
        switch self {
        case .gemini, .openai, .anthropic, .groq:
            let configuredModels = LLMConfig.shared.workflowModels(for: registryId)
        ...
        case .gemini:
            return [
                WorkflowModelOption(id: "gemini-2.0-flash-lite", name: "Gemini 2.0 Flash Lite", contextWindow: 1000000,
                ...
                WorkflowModelOption(id: "gemini-2.5-pro-latest", name: "Gemini 2.5 Pro", contextWindow: 1000000,
                                   costTier: .capable, inputCostPer1M: 1.25, outputCostPer1M: 10.00),
            ]
        case .openai:
            return [
                ...
                WorkflowModelOption(id: "gpt-5.2-chat-latest", name: "GPT-5.2", contextWindow: 128000,
                                   costTier: .balanced),
                WorkflowModelOption(id: "o4-mini", name: "o4-mini", contextWindow: 200000,
                                   costTier: .balanced),
                WorkflowModelOption(id: "gpt-5.2-pro", name: "GPT-5.2 Pro", contextWindow: 128000,
                                   costTier: .capable),
            ]
        case .anthropic:
            return [
                WorkflowModelOption(id: "claude-haiku-4-5-20251001", name: "Claude 4.5 Haiku", contextWindow: 200000,
                                   costTier: .budget, inputCostPer1M: 0.80, outputCostPer1M: 4.00, maxOutputTokens: 8192),
                WorkflowModelOption(id: "claude-sonnet-4-6-20250514", name: "Claude Sonnet 4.6", contextWindow: 200000,
                                   costTier: .balanced, inputCostPer1M: 3.00, outputCostPer1M: 15.00, maxOutputTokens: 16384),
                WorkflowModelOption(id: "claude-opus-4-6-20250514", name: "Claude Opus 4.6", contextWindow: 200000,
                                   costTier: .capable, inputCostPer1M: 15.00, outputCostPer1M: 75.00, maxOutputTokens: 32768),
            ]
        case .groq:
            return [
                WorkflowModelOption(id: "llama-3.3-70b-versatile", name: "Llama 3.3 70B", contextWindow: 128000,
                                   costTier: .budget, inputCostPer1M: 0.59, outputCostPer1M: 0.79),
        ...
        case .grok:
            return [
                WorkflowModelOption(id: "grok-3-mini-fast", name: "Grok 3 Mini Fast", contextWindow: 131072,
                                   costTier: .budget, inputCostPer1M: 0.30, outputCostPer1M: 0.50),
                WorkflowModelOption(id: "grok-3-mini", name: "Grok 3 Mini", contextWindow: 131072,
...
struct LLMStepConfig: Codable {
    ...
    func resolveProviderAndModel(
        ...
        if let explicitProvider = provider, explicitProvider != .mlx {
...
struct ShellStepConfig: Codable {
    ...
    static var customAllowedExecutables: [String] {
        WorkflowConfigurationStore.shared.configuration.runtime.customAllowedExecutables
    }
...
struct WebhookStepConfig: Codable {
    ...
    var auth: WebhookAuth?           // Optional authentication
...
enum WebhookAuth: Codable {
    case bearer(credentialId: UUID)                    // Authorization: Bearer <token>
    case apiKey(header: String, credentialId: UUID)    // Custom header with API key

    var credentialId: UUID {
        switch self {
        case .bearer(let id): return id
        case .apiKey(_, let id): return id
        }
    }

    // Custom Codable for associated values
    private enum CodingKeys: String, CodingKey {
        case type, credentialId, header
    }
    ...
    init(from decoder: Decoder) throws {
        ...
        let credentialId = try container.decode(UUID.self, forKey: .credentialId)

        switch type {
        case .bearer:
            self = .bearer(credentialId: credentialId)
        case .apiKey:
            let header = try container.decode(String.self, forKey: .header)
            self = .apiKey(header: header, credentialId: credentialId)
        }
    }
    ...
    func encode(to encoder: Encoder) throws {
        ...
        case .apiKey(let header, let credentialId):
...
// MARK: - Cloud Upload Step Configuration

/// Configuration for uploading audio to cloud storage (S3/R2)
struct CloudUploadStepConfig: Codable {
    var provider: CloudStorageProvider
    var bucket: String
    var region: String?
    var endpoint: String?            // Custom endpoint (required for R2, optional for S3)
    var pathTemplate: String         // e.g., "audio/{{MEMO_ID}}.m4a"
    var credentialId: UUID?          // Reference to secure credential in CredentialStore
    var contentType: String          // MIME type for upload
    ...
    init(
        ...
        region: String? = nil,
        endpoint: String? = nil,
        pathTemplate: String = "audio/{{MEMO_ID}}.m4a",
...
enum CloudStorageProvider: String, Codable, CaseIterable {
    case s3 = "s3"
    case r2 = "r2"
...
struct SaveFileStepConfig: Codable {
    ...
    static var defaultOutputDirectory: String {
        get {
            let saved = WorkflowConfigurationStore.shared.configuration.runtime.defaultOutputDirectory
            if !saved.isEmpty {
                return saved
            }
        ...
        set {
            WorkflowConfigurationStore.shared.updateRuntime { runtime in
                runtime.defaultOutputDirectory = newValue
            }
        }
    }
    ...
    static var pathAliases: [String: String] {
        get {
            WorkflowConfigurationStore.shared.configuration.runtime.pathAliases
        }
        set {
            WorkflowConfigurationStore.shared.updateRuntime { runtime in
                runtime.pathAliases = newValue
            }
        }
    }
...
enum TTSProvider: String, Codable, CaseIterable {
    ...
    case speakeasy = "speakeasy"    // Legacy alias — routes to OpenAI
    case openai = "openai"          // OpenAI TTS (direct API)
    case elevenlabs = "elevenlabs"  // ElevenLabs (direct API)
    case local = "local"            // Legacy local provider kept for workflow decoding

    var displayName: String {
        switch self {
        case .system: return "System (macOS)"
        case .speakeasy: return "OpenAI"
        case .openai: return "OpenAI"
        case .elevenlabs: return "ElevenLabs"
        case .local: return "Legacy Local (Removed)"
        }
    }
}
...
struct SpeakStepConfig: Codable {
    ...
    var useCache: Bool                  // Cache for repeated phrases
    ...
    static func availableVoices(for provider: TTSProvider) -> [(id: String, name: String)] {
        switch provider {
        case .system:
            return TTSVoiceCatalog.systemVoiceOptions()
...
/// Identifies the origin of a workflow
enum WorkflowSource: Codable, Equatable {
    case system                        // Protected system workflow (Transcribe, Hey Talkie)
    case user                          // User-created workflow
    case imported(ImportMetadata)      // Imported via URL

    var isEditable: Bool {
        switch self {
        case .system: return false
        case .user, .imported: return true
        }
    }
    ...
    /// Get import metadata if this is an imported workflow
    var importMetadata: ImportMetadata? {
        if case .imported(let metadata) = self {
            return metadata
        }
        return nil
    }

    // Custom Codable for enum with associated value
    private enum CodingKeys: String, CodingKey {
        case type, metadata
    }

    private enum SourceType: String, Codable {
        case system, user, imported
    }
    ...
    init(from decoder: Decoder) throws {
        ...
        case .user:
            self = .user
        case .imported:
            let metadata = try container.decode(ImportMetadata.self, forKey: .metadata)
    ...
    func encode(to encoder: Encoder) throws {
        ...
        switch self {
        case .system:
            try container.encode(SourceType.system, forKey: .type)
        case .user:
            try container.encode(SourceType.user, forKey: .type)
        case .imported(let metadata):
            try container.encode(SourceType.imported, forKey: .type)
            try container.encode(metadata, forKey: .metadata)
        }
    }
}

/// Metadata about an imported workflow
struct ImportMetadata: Codable, Equatable {
    let sourceUrl: URL              // Where it was imported from
    let importedAt: Date            // When it was imported
    let fingerprint: String         // SHA256 hash for deduplication/updates

    init(sourceUrl: URL, importedAt: Date = Date(), fingerprint: String) {
