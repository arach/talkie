...
//  Talkie
//
//  Health status cards for Agent and Engine services
...
// MARK: - Agent Recording Health Card
...
struct LiveRecordingHealthCard: View {
    ...
    var body: some View {
        ServiceHealthCard(
            ...
            detailText: liveState.isRunning
                ? "Agent Mode is active and ready to capture dictation."
                : "Agent Mode is inactive. Enable Agent Mode to use hotkeys and instant capture.",
            action: liveState.isRunning ? nil : {
                // Launch TalkieAgent
                launchTalkieAgent()
            },
            actionLabel: "Enable Agent Mode"
        )
        .onAppear {
    ...
    private func launchTalkieAgent() {
        if let appURL = NSWorkspace.shared.urlForApplication(withBundleIdentifier: ServiceManager.liveBundleId) {
...
struct EngineHealthCard: View {
    ...
    @Environment(AgentSettings.self) private var liveSettings

    /// Last error from TalkieAgent logs (read from log file)
            ...
            if let error = recentError {
                ...
                .popover(isPresented: $showErrorPopover, arrowEdge: .trailing) {
                    VStack(alignment: .leading, spacing: Spacing.sm) {
                        ...
                        Text("Try: Restart TalkieAgent (⌘Q then relaunch)")
                            .font(Theme.current.fontXS)
                            .foregroundColor(Theme.current.foregroundSecondary)
    ...
    private var detailText: String {
        ...
        case .running:
            if recentError != nil {
                return "TalkieAgent transcription is running but encountered an error."
            }
            return "TalkieAgent transcription is running with \(modelName) model."
        case .stopped:
            return "TalkieAgent is not running, so local transcription is unavailable."
        case .launching:
            return "TalkieAgent transcription is starting up..."
        case .terminating:
            return "TalkieAgent is shutting down..."
        case .unknown:
            return "The transcription state is unknown."
        }
    }
    ...
    /// Check TalkieAgent logs for recent XPC errors
    private func checkForRecentErrors() {
        // Load recent errors from TalkieAgent logs
...
#Preview("Healthy") {
    VStack(spacing: Spacing.md) {
        ServiceHealthCard(
            ...
            isHealthy: true,
            statusText: "Ready",
            detailText: "Talkie Agent is running and ready to capture dictation."
        )
        ...
        ServiceHealthCard(
            ...
            isHealthy: true,
            statusText: "Ready",
            detailText: "TalkieAgent transcription is running with Whisper Large v3 model."
        )
    }
    .padding()
    .frame(width: 500)
