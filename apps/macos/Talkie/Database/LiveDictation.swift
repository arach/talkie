...
struct LiveDictation: Identifiable, Hashable {
    var id: Int64?
    var recordingID: String?  // UUID string from recordings table (new unified schema)
    var createdAt: Date
    var text: String
    var mode: String
    ...
    var perfInAppMs: Int?              // TalkieAgent in-app processing (endToEnd - engine)
    ...
    var createdInTalkieView: Bool   // Was this recorded inside Talkie Agent UI?
    var pasteTimestamp: Date?       // When was this pasted? (nil = still queued)

    static let databaseTableName = "dictations"

    enum Columns: String, ColumnExpression {
        case id, recordingID, createdAt, text, mode, appBundleID, appName, windowTitle
...
extension LiveDictation: FetchableRecord {
    init(row: Row) {
        id = row[Columns.id]
        recordingID = nil  // Old dictations table doesn't have UUID
...
extension LiveDictation: PersistableRecord {
    func encode(to container: inout PersistenceContainer) {
        ...
        // recordingID is not persisted to dictations table (only used when reading from recordings table)
