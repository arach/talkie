...
/// A view that REQUIRES AgentSettings from the environment
/// This will crash if AgentSettings is not provided via .environment()
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            Text("""
                ...
                @Environment(AgentSettings.self) requires the value to be
                provided via .environment(). If it's missing, SwiftUI crashes
                with: "No Observable object of type AgentSettings found"
                """)
                .font(.caption)
        ...
        .sheet(isPresented: $showSafeSheet) {
            ViewThatNeedsEnvironment()
                .environment(AgentSettings.shared)  // ✅ Environment provided
        ...
        .sheet(isPresented: $showUnsafeSheet) {
            ViewThatNeedsEnvironment()
                // ❌ NO .environment(AgentSettings.shared) - CRASH!
                .frame(width: 300, height: 200)
        }
    }
}
...
struct EnvironmentCrashImmediateView: View {
    // This will crash because AgentSettings is not in the environment
    @Environment(AgentSettings.self) private var liveSettings
...
extension EnvironmentCrashTestView {
    ...
    static func triggerImmediateCrash() {
        ...
        // This will crash! The view expects AgentSettings in environment but we don't provide it
        window.contentView = NSHostingView(rootView: EnvironmentCrashImmediateView())
        // ❌ NO .environment(AgentSettings.shared) - should crash here
