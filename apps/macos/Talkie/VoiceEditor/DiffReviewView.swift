...
struct DiffReviewView: View {
    ...
    // Optional LLM info (passed by caller)
    var lastPrompt: String?
    var lastUsedProvider: String?
            ...
            if let lastPrompt = lastPrompt {
                DisclosureGroup(isExpanded: $showPromptDetails) {
                    ...
                    HStack(spacing: 6) {
                        ...
                        // Model name
                        if let provider = lastUsedProvider, let model = lastUsedModel {
                            Text("\(provider) / \(model)")
                                .font(Theme.current.fontXS)
                                .foregroundColor(textMuted)
                        }
    ...
    private var providerColor: Color {
        guard let providerId = llmProviderId else { return textMuted }
        switch providerId {
    ...
    private var footerBar: some View {
        VStack(spacing: Spacing.sm) {
            ...
            if let voiceInstruction = voiceInstruction {
