...
final class ListKeyboardNavigator<ItemID: Hashable> {
    ...
    @ObservationIgnored
    private let keyMonitorStorage = KeyMonitorStorage()
    ...
    deinit {
        ...
        if let monitor = keyMonitorStorage.monitor {
    ...
    private func setupKeyMonitor() {
        keyMonitorStorage.monitor = NSEvent.addLocalMonitorForEvents(matching: .keyDown) { [weak self] event in
    ...
    private func handleKeyEvent(_ event: NSEvent) -> NSEvent? {
        ...
        case 36: // Return/Enter
            // Yield to voice command overlay when active
            if SettingsManager.shared.isVoiceCommandPresented {
                return event
            }
            activateFocused()
            return nil
...
private final class KeyMonitorStorage: @unchecked Sendable {
    var monitor: Any?
}
