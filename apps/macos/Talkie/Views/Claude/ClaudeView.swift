...
struct ClaudeSession: Codable, Identifiable, Hashable {
    ...
    var lastSeenDate: Date? {
        TalkieDate.fromISO8601(lastSeen)
    }
}
...
struct ClaudeView: View {
                            ...
                            if isWide {
                                HStack(alignment: .top, spacing: 16) {
                                    ...
                                    metadataSection("Session") {
                                        ...
                                        metadataRow("Status", meta.session.isLive ? "Agent" : "Inactive")
                                    }
    ...
    private func loadLiveContext(for sessionId: String) {
        // Read from TalkieAgent's Application Support folder
        ...
        Task {
            do {
                guard let xpc = ServiceManager.shared.live.xpcManager,
                    ...
                    throw NSError(domain: "Claude", code: 1, userInfo: [NSLocalizedDescriptionKey: "Talkie Agent not connected"])
                }
    ...
    private func formatTimestamp(_ iso: String) -> String {
        guard let date = TalkieDate.fromISO8601(iso) else { return iso }
...
private struct MessageRow: View {
    ...
    private func formatTime(_ iso: String) -> String {
        guard let date = TalkieDate.fromISO8601(iso) else { return "" }
        let df = DateFormatter()
