...
//  Talkie macOS
//
//  Introduces Agent mode features with status bar preview and shortcut selection
...
struct LiveModePitchView: View {
    ...
    private var liveSettings: AgentSettings { AgentSettings.shared }
            ...
            content: {
                VStack(spacing: Spacing.lg) {
                    ...
                    Text(manager.enableLiveMode ?
                        "Agent features can be disabled anytime in Settings" :
                        "You can enable Agent mode later in Settings")
                        .font(.system(size: 10))
