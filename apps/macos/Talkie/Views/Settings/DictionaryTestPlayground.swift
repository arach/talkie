...
//  Two-column layout: sample list (memos/dictations) | editor + output
//
...
struct DictionaryTestPlayground: View {
    ...
    @State private var recentDictations: [TalkieObject] = []
    ...
    // Column widths
    private static let sidebarWidth: (min: CGFloat, ideal: CGFloat, max: CGFloat) = (200, 240, 300)
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            HSplitView {
                ...
                sampleListPane
                    .frame(
                        minWidth: Self.sidebarWidth.min,
                        idealWidth: Self.sidebarWidth.ideal,
                        maxWidth: Self.sidebarWidth.max
                    )
    ...
    private var titleBar: some View {
        HStack {
            VStack(alignment: .leading, spacing: 2) {
                ...
                Text("Select a memo or dictation to test dictionary rules")
                    .font(.system(size: 11))
    ...
    private var sampleListPane: some View {
                ...
                ForEach(SampleFilter.allCases, id: \.self) { filter in
                    Button {
                        ...
                        Text(filter.rawValue)
                            .font(.system(size: 11, weight: sampleFilter == filter ? .semibold : .regular))
                            .foregroundColor(sampleFilter == filter ? Theme.current.accent : Theme.current.foregroundSecondary)
        ...
        .background(Theme.current.background)
        .onChange(of: selectedSampleId) { _, newValue in
            if let id = newValue, let sample = filteredSamples.first(where: { $0.id == id }) {
    ...
    private func sampleRow(_ sample: SampleItem) -> some View {
        VStack(alignment: .leading, spacing: 2) {
    ...
    private func metricsSection(_ metrics: ProcessingMetrics) -> some View {
        VStack(alignment: .leading, spacing: Spacing.sm) {
            ...
            HStack(spacing: Spacing.lg) {
                ...
                metricPill(
                    label: "Total",
                    value: formatMs(metrics.totalMs),
                    color: metrics.totalMs < 5 ? .green : (metrics.totalMs < 20 ? .orange : .red)
                )

                // Breakdown
                metricPill(label: "Trie", value: formatMs(metrics.trieMs), color: .blue)
                metricPill(label: "Regex", value: formatMs(metrics.regexMs), color: .purple)
                metricPill(label: "Fuzzy", value: formatMs(metrics.fuzzyMs), color: .cyan)
    ...
    /// Format milliseconds - show "< 1ms" for sub-millisecond, otherwise round to 1 decimal
    ...
    private var filteredSamples: [SampleItem] {
        ...
        // Add memos
        if sampleFilter == .all || sampleFilter == .memos {
            samples += recentMemos.compactMap { memo -> SampleItem? in
                guard let text = memo.transcription, !text.isEmpty else { return nil }
                return SampleItem(
        ...
        // Add dictations
        if sampleFilter == .all || sampleFilter == .dictations {
            samples += recentDictations.compactMap { recording -> SampleItem? in
                guard let text = recording.text, !text.isEmpty else { return nil }
                return SampleItem(
                    id: "dictation-\(recording.id.uuidString)",
                    title: recording.appContext?.name ?? "Dictation",
                    preview: String(text.prefix(80)),
                    text: text,
    ...
    private func loadSampleSources() async {
        ...
        do {
            ...
            let memos = try await repository.fetchTranscribedMemos(limit: 30)
            await MainActor.run {
                recentMemos = memos
            }
        } catch {
        ...
        do {
            let recordingRepo = TalkieObjectRepository()
            let recordings = try await recordingRepo.fetchRecentDictations(limit: 30)
            // Filter to only those with non-empty text
            let withText = recordings.filter { ($0.text ?? "").isEmpty == false }
            await MainActor.run {
