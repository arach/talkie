...
struct ServerSettingsView: View {
    ...
    var body: some View {
        SettingsPageContainer {
            ...
            VStack(alignment: .leading, spacing: 16) {
                ServerAccessSection(
                    serverEnabled: serverEnabledBinding,
                    gatewayEnabled: gatewayEnabledBinding,
                    claudeEnabled: claudeEnabledBinding,
                    isRunning: bridgeManager.bridgeStatus == .running
                )
                ...
                ServerProcessSection(
                    ...
                    isEnabled: settingsManager.talkieServerEnabled,
        ...
        .alert("Enable TalkieServer?", isPresented: $showEnableAlert) {
            ...
            Text("This starts a local server for Bridge and optional Gateway modules. Enable only when you intend to use iOS connectivity or external API routing.")
        }
        .alert("Enable Claude Code sessions?", isPresented: $showClaudeEnableAlert) {
            Button("Enable") {
                settingsManager.talkieClaudeSessionsEnabled = true
            }
            Button("Cancel", role: .cancel) {}
        } message: {
            Text("This scans ~/.claude/projects and builds a cache for session browsing. Only enable if you need Claude Code session access.")
        }
    }
    ...
    private var gatewayEnabledBinding: Binding<Bool> {
        Binding(
            get: { settingsManager.talkieGatewayEnabled },
            set: { newValue in
                settingsManager.talkieGatewayEnabled = newValue
            }
        )
    }
    ...
    private var claudeEnabledBinding: Binding<Bool> {
        Binding(
            get: { settingsManager.talkieClaudeSessionsEnabled },
            set: { newValue in
                if newValue {
                    showClaudeEnableAlert = true
                } else {
                    settingsManager.talkieClaudeSessionsEnabled = false
                }
            }
        )
    }
...
private struct ServerAccessSection: View {
    let serverEnabled: Binding<Bool>
    let gatewayEnabled: Binding<Bool>
    let claudeEnabled: Binding<Bool>
    let isRunning: Bool
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Toggle(isOn: gatewayEnabled) {
                VStack(alignment: .leading, spacing: 4) {
                    Text("Enable Gateway module")
                        .font(Theme.current.fontSMMedium)
                        .foregroundColor(Theme.current.foreground)
                    Text("Adds external API routing (requires restart).")
                        .font(Theme.current.fontXS)
                        .foregroundColor(Theme.current.foregroundSecondary)
                }
            }
            .toggleStyle(.switch)
            .disabled(!serverEnabled.wrappedValue)

            Toggle(isOn: claudeEnabled) {
                VStack(alignment: .leading, spacing: 4) {
                    Text("Enable Claude Code sessions")
                        .font(Theme.current.fontSMMedium)
                        .foregroundColor(Theme.current.foreground)
                    Text("Scans and caches Claude sessions (requires restart).")
                        .font(Theme.current.fontXS)
                        .foregroundColor(Theme.current.foregroundSecondary)
                }
            }
            ...
            if isRunning {
                Text("Restart TalkieServer to apply module changes.")
                    .font(Theme.current.fontXS)
                    .foregroundColor(Theme.current.foregroundSecondary)
            }
        }
...
private struct ServerProcessSection: View {
            ...
            HStack(spacing: 12) {
                ...
                HStack(spacing: 8) {
                    if status == .running {
                        ...
                        .buttonStyle(.plain)
                        .disabled(!tailscaleReady || !isEnabled)
    ...
    private var statusDescription: String {
        if !isEnabled {
            return "Server is locked - enable TalkieServer to start"
        }
        switch status {
