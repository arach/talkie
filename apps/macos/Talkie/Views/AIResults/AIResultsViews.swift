        ...
        Button(action: onSelect) {
            HStack(spacing: 10) {
                ...
                Image(systemName: workflowIcon)
                    .font(Theme.current.fontSM)
                    .foregroundColor(isSelected ? .white : (run.isFailed ? SemanticColor.error : .secondary))
                    .frame(width: 22, height: 22)
                    .background(isSelected ? settings.resolvedAccentColor : (run.isFailed ? SemanticColor.error.opacity(0.12) : Color.primary.opacity(0.05)))
                    .cornerRadius(CornerRadius.xs)
                ...
                VStack(alignment: .leading, spacing: 2) {
                    HStack(spacing: 6) {
                        ...
                        if run.isFailed {
                            Text("FAILED")
                                .font(Theme.current.fontXS)
                                .foregroundColor(isSelected ? .white.opacity(0.8) : SemanticColor.error)
                        }
                    }
