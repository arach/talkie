...
//
//  This was the standalone TalkieAgent navigation shell with its own sidebar.
...
struct HistoryView: View {
    ...
    private var filteredDictations: [Dictation] {
        ...
        case .queue:
            ...
            // DEPRECATED: Queue filtering disabled - this view is deprecated
            // TODO: Remove .queue case entirely when HistoryView is refactored
            result = []
        case .today:
            // Items from today
    ...
    private func extractAudioMetadata(from url: URL, originalFilename: String) -> (duration: Double?, metadata: [String: String]) {
        ...
        if let attrs = try? FileManager.default.attributesOfItem(atPath: url.path) {
            ...
            if let created = attrs[.creationDate] as? Date {
                metadata["fileCreatedAt"] = created.iso8601
            }
            if let modified = attrs[.modificationDate] as? Date {
                metadata["fileModifiedAt"] = modified.iso8601
            }
        }
        ...
        VStack(spacing: Spacing.xxs) {
            ...
            ScrollView(.vertical, showsIndicators: false) {
                VStack(spacing: Spacing.xxs) {
                    ...
                    // DEPRECATED: Queue count disabled - this view is deprecated
                    // TODO: Remove queue nav item when HistoryView is refactored
                    SidebarNavItem(
    ...
    private var emptyHistoryState: some View {
        VStack(spacing: Spacing.sm) {
            ...
            Text("No Recordings Yet")
                ...
                .foregroundColor(TalkieTheme.textSecondary)
            Text("Press \(AgentSettings.shared.hotkey.displayString) to start recording")
                .font(Design.fontSM)
                .foregroundColor(TalkieTheme.textMuted)
            Spacer()
        }
    ...
    private var settingsDetailPlaceholder: some View {
        VStack(spacing: Spacing.sm) {
            ...
            Text("Configure Talkie Agent settings")
                .font(Design.fontSM)
                .foregroundColor(TalkieTheme.textMuted)
        }
        .frame(maxWidth: .infinity, maxHeight: .infinity)
...
private struct JSONContentView: View {
    ...
    func renderedString() -> String {
        ...
        appendLine(&lines, key: "timestamp", value: "\"\(dictation.timestamp.iso8601)\"")
...
private struct PerformanceCard: View {
    ...
    // App: everything TalkieAgent does (file save, context, routing, paste)
    ...
    private func openEngineTrace(_ sessionID: String) {
        ...
        ServiceManager.shared.openHelperRoute(.agent, route: "trace/\(sessionID)")
    }
...
private struct SmartActionsCard: View {
    ...
    // DEPRECATED: This view is deprecated - returning nil
    // TODO: Remove SmartActionsCard entirely when HistoryView is refactored
