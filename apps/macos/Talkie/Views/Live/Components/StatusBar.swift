...
struct StatusBar: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
                ...
                HStack(spacing: 8) {
                    if liveState.isRunning {
                        ...
                        if showPID, let pid = liveState.processId {
                            ...
                            .buttonStyle(.plain)
                            .help("Click to copy Talkie Agent PID")
                            .transition(.opacity.combined(with: .scale(scale: 0.9)))
                        }
                    } else {
                        LiveOfflineIcon(action: launchTalkieAgent)
                    }
                }
                ...
                HStack(spacing: Spacing.sm) {
                    ...
                    HStack(spacing: 6) {
                        if serviceMonitor.state == .running {
                            ...
                            .help("Transcription: \(serviceMonitor.state.rawValue). Click to launch TalkieAgent.")
                            .onTapGesture {
                                Task {
                                    if serviceMonitor.state != .running {
                        ...
                        Button(action: openAboutSettings) {
                            Text("v\(appVersion)")
                                ...
                                .background(
                                    RoundedRectangle(cornerRadius: CornerRadius.xs)
                                        .fill(TalkieTheme.hover.opacity(0.85))
                                )
                        }
                        .buttonStyle(.plain)
            ...
            .padding(.leading, 23) // Align sync icon center with sidebar icon column (32pt)
