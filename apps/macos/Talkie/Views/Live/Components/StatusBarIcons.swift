...
struct LiveStatusIcon: View {
    ...
    private var statusColor: Color {
        ...
        // Note: Silence detection happens in TalkieAgent, not here
    ...
    private var label: String {
        if !liveState.isRunning { return "Agent" }
        return "Built-in"  // TODO: wire to actual device name
    }

    private var detail: String {
        if !liveState.isRunning {
            return "Agent Mode is inactive - Click to open Agent settings"
        }
    ...
    private func navigateToSettings() {
        if liveState.isRunning {
            ...
            NotificationCenter.default.post(name: .switchToAgentSettingsAudio, object: nil)
        } else {
            // Navigate to Live Settings → Overview to see health card
            NotificationCenter.default.post(name: .switchToAgentSettingsOverview, object: nil)
        }
    }
}
...
struct SimpleEngineIcon: View {
    ...
    var body: some View {
        ...
        .buttonStyle(.plain)
        .onHover { isHovered = $0 }
        .help("Transcription - Click for helper settings")
    }
...
struct EngineStatusIcon: View {
    ...
    var body: some View {
        HStack(spacing: 6) {
            ...
            .buttonStyle(.plain)
            .delayedHelp("Transcription - Click for the Agent dashboard, Shift+Click to launch or restart TalkieAgent", delay: 0.6)
    ...
    private func navigateToEngineSettings() {
        ServiceManager.shared.openHelperRoute(.agent, route: "dashboard")
    }
}
...
struct ModelStatusIcon: View {
    ...
    private func navigateToModelSettings() {
        ...
        NotificationCenter.default.post(name: .switchToAgentSettingsTranscription, object: nil)
    }
}
...
struct SyncStatusIcon: View {
    ...
    var body: some View {
        ...
        .popover(isPresented: $showingPopover, arrowEdge: .bottom) {
            SyncPanel(onExpand: expandToSheet)
        }
    ...
    private func expandToSheet() {
        showingPopover = false
        // Small delay so the popover finishes dismissing before sheet appears
