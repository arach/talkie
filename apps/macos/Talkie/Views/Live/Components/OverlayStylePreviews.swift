...
// Miniature version of WavyParticlesView from TalkieAgent
...
struct WavyParticlesPreview: View {
    ...
    var body: some View {
        ZStack {
            ...
            TimelineView(.animation(minimumInterval: 0.033)) { timeline in
                Canvas { context, size in
                    ...
                    // Constrain amplitude to stay within bounds (with particle radius margin)
                    let maxAmplitude = (size.height / 2) - 4
                    let amplitude: CGFloat = calm ? min(5, maxAmplitude) : min(12, maxAmplitude)
        ...
        .onAppear {
            log.debug("WavyParticlesPreview appeared (calm: \(calm))")
        }
    }
}

// MARK: - Waveform Bars Preview
// Miniature version of WaveformBarsView from TalkieAgent
...
struct WaveformBarsPreview: View {
    ...
    var body: some View {
        ...
        .clipShape(RoundedRectangle(cornerRadius: CornerRadius.xs))
        .onAppear {
            log.debug("WaveformBarsPreview appeared (sensitive: \(sensitive))")
        }
    }
}
