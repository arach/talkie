...
final class MicCaptureService: @unchecked Sendable {
    ...
    func startSession(
        ...
        await MainActor.run {
            IdleWatchdog.shared.setActiveSessionCount(activeSessionCount)
        }

        log.info("TalkieMic started session", detail: "\(sessionId) client=\(clientId)")
        return sessionId
    }
    ...
    private func ensureEngineRunning() throws {
        ...
        log.info("TalkieMic audio engine started", detail: selection.name)
    }
    ...
    private func stopEngine() {
        ...
        engineToStop.stop()
        log.info("TalkieMic audio engine stopped")
    }
    ...
    private func expireStaleSessions() async {
        ...
        for sessionId in expired {
            ...
            log.warning("TalkieMic expired stale session", detail: sessionId)
        }
    }
