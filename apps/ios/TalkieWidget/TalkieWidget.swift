...
enum TalkieWidgetRuntimeIdentifiers {
    static var appGroupIdentifier: String {
        guard let value = Bundle.main.object(forInfoDictionaryKey: "TalkieAppGroupIdentifier") as? String else {
            return "group.to.talkie.app.ios"
        }

        let trimmed = value.trimmingCharacters(in: .whitespacesAndNewlines)
        return trimmed.isEmpty ? "group.to.talkie.app.ios" : trimmed
    }
}
...
struct Provider: TimelineProvider {
    ...
    private func getMemoCount() -> Int {
        let defaults = UserDefaults(suiteName: TalkieWidgetRuntimeIdentifiers.appGroupIdentifier)
        return defaults?.integer(forKey: "memoCount") ?? 0
    }

    private func getRecentMemos() -> [MemoSnapshot] {
        guard let defaults = UserDefaults(suiteName: TalkieWidgetRuntimeIdentifiers.appGroupIdentifier),
