...
class SessionSummaryCache: ObservableObject {
    ...
    private static func generateSummaryInBackground(sessionId: String) async {
        ...
        let isAvailable = aiService.isAvailable

        log.debug("SessionSummary: Attempting for \(sessionId), AI available: \(isAvailable)")

        guard isAvailable else {
            log.debug("SessionSummary: AI not available, skipping")
            await MainActor.run {
                _ = shared.pendingSessionIds.remove(sessionId)
            }
            return
        }
        ...
        do {
            ...
            log.debug("SessionSummary: Got \(messages.count) messages")

            guard !messages.isEmpty else {
                log.debug("SessionSummary: No messages, skipping")
                await MainActor.run {
                    _ = shared.pendingSessionIds.remove(sessionId)
                }
                return
            ...
            log.debug("SessionSummary: Generated summary")
            ...
            log.warning("SessionSummary: Error: \(error.localizedDescription)")
            await MainActor.run {
                _ = shared.pendingSessionIds.remove(sessionId)
            }
        }
...
struct SessionRow: View {
    ...
    private var isRecent: Bool {
        guard let date = TalkieDate.fromISO8601( session.lastSeen) else { return false }
    ...
    private var compactTime: String {
        guard let date = TalkieDate.fromISO8601( session.lastSeen) else { return "" }
...
struct ProjectPathSection: View {
    ...
    private var visibleSessions: [PathSession] {
        ...
        let uniqueSessions = projectPath.sessions.reduce(into: [String: PathSession]()) { dict, session in
            ...
            let date1 = TalkieDate.fromISO8601(session1.lastSeen) ?? Date.distantPast
            let date2 = TalkieDate.fromISO8601(session2.lastSeen) ?? Date.distantPast
            return date1 > date2
        }
    ...
    private func compactTime(from isoString: String) -> String {
        guard let date = TalkieDate.fromISO8601( isoString) else { return "" }
        let interval = Date().timeIntervalSince(date)
...
struct PathSessionRow: View {
    ...
    private var lastSeenDate: Date? {
        return TalkieDate.fromISO8601(session.lastSeen)
    }
