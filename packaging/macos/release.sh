...
if [ -z "${TALKIE_DEVELOPER_ID_APP:-}" ]; then
    echo "ERROR: TALKIE_DEVELOPER_ID_APP is not configured"
    echo "Set TALKIE_SIGNING_ENV_FILE or source a private signing env file before release."
    exit 1
fi

if ! security find-identity -v | grep -Fq "$TALKIE_DEVELOPER_ID_APP"; then
    echo "ERROR: Developer ID Application certificate not found: $TALKIE_DEVELOPER_ID_APP"
    exit 1
fi
...
if ! xcrun notarytool history --keychain-profile "$NOTARY_PROFILE" >/dev/null 2>&1; then
    ...
    echo "Run: xcrun notarytool store-credentials \"$NOTARY_PROFILE\" --apple-id YOUR_APPLE_ID --team-id YOUR_TEAM_ID"
    exit 1
fi
