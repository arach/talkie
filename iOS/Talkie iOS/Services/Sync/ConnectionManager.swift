...
private let logger = Logger(subsystem: "jdi.talkie.ios", category: "ConnectionManager")
...
class ConnectionManager {
    ...
    func register(_ provider: any SyncProvider) {
        ...
        logger.info("Registered sync provider: \(provider.method.rawValue)")
    }
        ...
        for method in preferredMethods {
            ...
            if let provider = providers[method],
               methodStatus[method] == .available {
                ...
                logger.info("Active sync provider: \(method.rawValue)")
                return
            }
        }
        // Fallback to local
    ...
    func sync() async throws {
        guard let provider = activeProvider else {
            logger.warning("No active sync provider")
            return
        }
