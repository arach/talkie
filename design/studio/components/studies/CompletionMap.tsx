...
const MILESTONES: Milestone[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { code: "N05", title: "Multi-account", detail: "WorkspaceSwitcherNext paint — identity list with active highlight, SWITCH chips, ACTIVE badge, role pills (Personal/Work), add-account dashed tile routing to SignIn. Codex wires WorkspaceStore.identities from Keychain ASAuthorization entries + activate(_:) which swaps iCloud zone + Core Data store + bridge pairing.", status: "in-flight", size: "xl", risk: "high" },
  ...
  },
  {
    ...
    items: [
      { code: "S01", title: "Accessibility audit", detail: "Phase A · VoiceOver labels (shipped). Phase B · Dynamic Type scaling (shipped). Phase C · ThemeContrastDebugNext debug surface landed — renders every theme's primary/secondary/tertiary/accent against background with computed WCAG 2.x contrast ratios and PASS/FAIL pills at 4.5:1 (3:1 large). Settings LAB → 'Inspect theme contrast'. Remaining: act on the failures.", status: "in-flight", size: "l", risk: "med" },
      ...
      { code: "S03", title: "Launch performance", detail: "Phase telemetry landed — talkieApp.init logs each boot phase (bg-task-register · app-settings-load · workflow-mirror-sync · theme-override-check) with its own ms cost and cumulative t= marker. Existing total init + total boot logs preserved. Codex Instruments profiling + trim work still ahead.", status: "in-flight", size: "m", risk: "med" },
