...
// ─── Shape B · Anchored pill, drop-down strip (current Swift target) ──
//
// Mirrors `apps/macos/Talkie/Components/TalkieChromeBar.swift`:
//   - Small dark capsule pinned top-center
//   - Circular pill with TALKIE caps-mono label below
//   - Hover: strip of 7 chip+label slots drops in below the pill,
//     contained inside the same capsule (capsule grows downward)
//   - Click 1 arms (red pulse, 2s window), click 2 starts recording
//   - Spring pop on reveal (cubic-bezier overshoot)
//
// Studio scale = 2× of native (small chrome reads better at zoom).
// Native is 24pt chips; studio renders at 48px so type + spacing
// stay legible while preserving proportions.

type PillStatus = "idle" | "armed" | "recording";

const V3_NAV_SLOTS: { icon: string; label: string; selected?: boolean }[] = [
  { icon: "⌂", label: "Home", selected: true },
  { icon: "▤", label: "Library" },
  { icon: "✎", label: "Compose" },
  { icon: "🗒", label: "Notes" },
  { icon: "〰", label: "Dictations" },
  { icon: "✺", label: "Workflows" },
  { icon: "⚙︎", label: "Settings" },
];
...
const V3 = {
  ...
  chipSpacing: 12,
  capsuleRadius: 32,
  capsuleBg: "rgba(20,16,12,0.95)",
  capsuleBorder: "rgba(255,255,255,0.10)",
  labelOpacity: 0.55,
  pillGradient: "linear-gradient(180deg, rgba(255,255,255,0.16), rgba(255,255,255,0.04))",
};

function AnchoredPillDropdown() {
  const [hovered, setHovered] = React.useState(false);
  const [status, setStatus] = React.useState<PillStatus>("idle");
  const armResetRef = React.useRef<ReturnType<typeof setTimeout> | null>(null);

  function clearArm() {
    if (armResetRef.current) {
      clearTimeout(armResetRef.current);
  ...
  function handlePillClick() {
    ...
    setStatus("armed");
    clearArm();
    armResetRef.current = setTimeout(() => {
      setStatus((s) => (s === "armed" ? "idle" : s));
    }, 2000);
  }

  React.useEffect(() => () => clearArm(), []);

  return (
    <section>
      <SectionHeader
        eyebrow="· Four · Shape B · Current Swift target"
        title="Anchored pill — drop-down strip"
        hint="hover to drop the strip · click pill once to arm · click again to record"
      />

      <div className="flex flex-col items-center gap-4">
        {/* Simulated content surface so the chrome reads in context */}
        <div
        ...
        >
          {/* Faint placeholder content so the chrome doesn't float in a void */}
          <div
            className="absolute inset-x-12 bottom-12 top-32 rounded-md"
            style={{ background: PAPER, border: `0.5px dashed ${EDGE}` }}
          />
        ...
        <div className="flex w-[1100px] items-baseline justify-between font-mono text-[9px] uppercase tracking-[0.20em] text-studio-ink-faint">
          <span>state · {hovered ? "revealed" : "idle"} · pill · {status}</span>
          <span>cubic-bezier(0.34, 1.56, 0.64, 1) · 220ms · overshoot</span>
        </div>
      </div>
    </section>
  );
...
function V3ChromeBar({
  ...
  return (
    <div
      ...
      onMouseLeave={onLeave}
      style={{
        background: V3.capsuleBg,
        border: `0.5px solid ${V3.capsuleBorder}`,
        borderRadius: V3.capsuleRadius,
        padding: "10px 16px",
        boxShadow:
          "0 6px 18px rgba(0,0,0,0.25), 0 1px 3px rgba(0,0,0,0.18)",
        // Single transition driving the drop — pill stays anchored, the
        // strip's intrinsic height inside the capsule animates.
        transition: "all 220ms cubic-bezier(0.34, 1.56, 0.64, 1)",
      }}
    >
      <div className="flex flex-col items-center" style={{ gap: 10 }}>
        <V3Pill status={status} onClick={onPillClick} />

        <V3Strip revealed={hovered} />
      </div>
    </div>
  );
}
...
function V3Pill({
  ...
  return (
    <button
      ...
      style={{ gap: 5, background: "transparent", border: "none", padding: 0, cursor: "pointer" }}
    >
      <div
        style={{
          position: "relative",
          ...
          height: V3.chipSize,
          borderRadius: "50%",
          background: V3.pillGradient,
          boxShadow: `inset 0 0 0 ${isArmed || isRecording ? 2 : 1}px ${ring}`,
          transition: "all 180ms ease-out",
          animation: isArmed ? "v3-arm-pulse 900ms ease-in-out infinite" : "none",
        }}
      >
        <div
          style={{
      ...
      <span
        style={{
          ...
          color: `rgba(255,255,255,${V3.labelOpacity})`,
        }}
      >
        Talkie
      </span>
      <style>{`
        @keyframes v3-arm-pulse {
          0%, 100% { box-shadow: inset 0 0 0 2px rgba(220,40,40,0.65); }
          50%      { box-shadow: inset 0 0 0 2px rgba(220,40,40,0.30); }
        }
      `}</style>
    </button>
  );
...
function V3Strip({ revealed }: { revealed: boolean }) {
  return (
    <div
      style={{
        // Reserve space when revealed; collapse when not. The container
        // animates via transform so the cubic-bezier reads as a spring.
        maxHeight: revealed ? 80 : 0,
        opacity: revealed ? 1 : 0,
        transform: revealed ? "translateY(0) scale(1)" : "translateY(-6px) scale(0.92)",
        transformOrigin: "top center",
        overflow: "hidden",
        transition:
          "max-height 220ms cubic-bezier(0.34, 1.56, 0.64, 1)," +
          "opacity 180ms ease-out," +
          "transform 220ms cubic-bezier(0.34, 1.56, 0.64, 1)",
        pointerEvents: revealed ? "auto" : "none",
      }}
    >
      <div className="flex items-start" style={{ gap: V3.chipSpacing, paddingTop: 4 }}>
        {V3_NAV_SLOTS.map((slot) => (
          <V3StripSlot key={slot.label} slot={slot} />
        ))}
      </div>
    </div>
  );
...
function V3StripSlot({
  slot,
}: {
  slot: { icon: string; label: string; selected?: boolean };
}) {
  const [hovered, setHovered] = React.useState(false);

  const fgOpacity = slot.selected ? 0.95 : hovered ? 0.92 : 0.55;
  const bgOpacity = slot.selected ? 0.14 : hovered ? 0.09 : 0;
  const labelOpacity = slot.selected ? 0.85 : hovered ? 0.78 : 0.42;
    ...
    >
      <div
        style={{
          ...
          alignItems: "center",
          justifyContent: "center",
          color: `rgba(255,255,255,${fgOpacity})`,
          fontSize: 18,
