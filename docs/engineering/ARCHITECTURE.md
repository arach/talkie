...
This document describes the architecture patterns used across the Talkie macOS apps (Talkie and TalkieAgent). It serves as a guide for understanding the codebase and making consistent decisions when adding new features.
...
| **TalkieAgent** | Always-on companion | Real-time transcription, quick capture, floating UI |
...
```
TalkieAgent/
├── App/           # Entry points, AgentController, dependencies
...
**Current singletons (TalkieAgent):** LiveSettings, UtteranceStore, AgentController, EngineClient, AudioDeviceManager, and others
...
#### State Machines (TalkieAgent)

AgentController uses an explicit state machine:
...
### 4. Dependency Injection (TalkieAgent)

TalkieAgent uses constructor injection for its core controller:

```swift
// AgentDependencies.swift defines protocols
...
- Simpler than Core Data, appropriate for TalkieAgent's needs
...
Protocol-based dependencies (like in TalkieAgent's AgentController) enable mock injection.
...
| `TalkieAgent/App/AgentController.swift` | Core state machine, transcription pipeline |
| `TalkieAgent/App/AgentDependencies.swift` | Protocol definitions for DI |
| `TalkieAgent/Services/HotKeyManager.swift` | Global hotkey handling |
