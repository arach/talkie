...
TalkieAgent has two logging systems that sometimes log the same event:
...
1. **os.log (Logger)** - Apple's structured logging
   ...
   - Used throughout: `Logger(subsystem: "to.talkie.agent", category: "LiveController")`
...
private let logger = Logger(subsystem: "to.talkie.agent", category: "LiveController")
...
final class LogViewer: ObservableObject {
    ...
    private let subsystem = "to.talkie.agent"
...
**Remove/Refactor:**
- `TalkieAgent/Services/SystemEventManager.swift` → Delete or repurpose

**Audit for duplicate logging:**
- `TalkieAgent/App/LiveController.swift`
- `TalkieAgent/App/AppDelegate.swift`
- `TalkieAgent/Services/EngineClient.swift`
- `TalkieAgent/Services/Audio/AudioCapture.swift`

**Create:**
- `TalkieAgent/Debug/LogViewer.swift` - OSLogStore reader
- `TalkieAgent/Debug/LogViewerView.swift` - SwiftUI log viewer
...
|-----------|----------|----------|
| `LiveSettings` | `TalkieAgent/Models/LiveSettings.swift` | High |
| `UtteranceStore` | `TalkieAgent/Stores/UtteranceStore.swift` | High |
| `LiveController` | `TalkieAgent/App/LiveController.swift` | Medium |
...
Several TalkieAgent view files are very large:
...
1. [ ] Run: `grep -r "try?" --include="*.swift" TalkieAgent/ | wc -l` to get count
2. [ ] Export list with context: `grep -r -n "try?" --include="*.swift" TalkieAgent/`
