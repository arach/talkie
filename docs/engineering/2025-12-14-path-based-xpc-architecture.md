...
```
TalkieAgent: copyItem() completes → file closed
TalkieAgent: sends path over XPC
...
|-----------|-----------|-------------|
| TalkieAgent | Yes | Files live in `~/Library/Containers/jdi.talkie.agent/` |
...
- **TalkieAgent:** Owns audio storage, manages permanent archive
...
| `TalkieAgent/EngineClient.swift` | Uses `audioPath: String` |
