...
**Purpose**: Port TalkieAgent's delightful onboarding animations and interactions into Talkie's superior modular architecture.
...
### **Core + Live Mode**
- Talkie + TalkieAgent integration
...
- **Services**: Both TalkieAgent and TalkieEngine auto-launch in background when needed (no onboarding step)

**Determining Mode:**
Check if TalkieAgent.app exists in /Applications or is being installed alongside Talkie. If yes, use Core + Live mode. If no, use Core mode.

**⚠️ IMPORTANT: Services Are Invisible**
There is **no** Service Setup onboarding step. Services (TalkieAgent, TalkieEngine) auto-launch transparently in the background when needed. The user never sees a "launching services" screen - it just works.
...
The animation in TalkieAgent is already perfect. Your job is to **copy and integrate** the existing components, not rebuild them from scratch.

**Source Files to Port from TalkieAgent:**
...
**Target File:**
- `/Users/example/dev/talkie/apps/macos/Talkie/Views/Onboarding/WelcomeView.swift`

**Implementation Steps:**

1. **Copy Components**: Locate all animation files in TalkieAgent's onboarding and copy them to Talkie's project
2. **Add to Xcode Project**: Ensure all copied files are added to the Talkie target
3. **Update Imports**: Change any TalkieAgent-specific imports to Talkie equivalents if needed
...
6. **Test**: Run and verify the animation works exactly as it does in TalkieAgent
...
`/Users/example/dev/talkie/apps/macos/TalkieAgent/TalkieAgent/Views/OnboardingView.swift` (lines ~300-800 for the animation logic)
...
**Core + Live Mode (Only if TalkieAgent is being installed):**
...
**Note:** If not installing TalkieAgent, only show Microphone permission. Accessibility and Screen Recording are Live-specific features.
...
**Target File:**
- `/Users/example/dev/talkie/apps/macos/Talkie/Views/Onboarding/PermissionsSetupView.swift` (already exists, enhance)
...
```swift
var isLiveMode: Bool {
    // Check if TalkieAgent.app exists
    let liveAppPath = "/Applications/TalkieAgent.app"
...
Services (TalkieAgent and TalkieEngine) now auto-launch transparently in the background. There is no user-facing "Service Setup" step in the onboarding flow.
...
- **TalkieAgent**: Launches when user presses hotkey (Core + Live mode only)
...
**Target File:**
- `/Users/example/dev/talkie/apps/macos/Talkie/Views/Onboarding/ModelInstallView.swift` (already exists, enhance heavily)
...
- No direct equivalent in TalkieAgent (this is Talkie-specific)
- Reference provider card pattern from ModelInstallView
- Reference secure input from macOS keychain examples

**Target File:**
- `/Users/example/dev/talkie/apps/macos/Talkie/Views/Onboarding/LLMConfigView.swift` (already exists, enhance)
...
The interactive pill demo and celebration animations already exist in TalkieAgent and are polished. Your job is to **copy and integrate** them into Talkie's CompleteView, not rebuild from scratch.
...
**Target File:**
- `/Users/example/dev/talkie/apps/macos/Talkie/Views/Onboarding/CompleteView.swift` (already exists, major enhancements needed)
...
1. **Copy Interactive Components**: Locate `OnboardingPillDemo` and `CelebrationView` in TalkieAgent and copy to Talkie
2. **Add to Xcode Project**: Ensure all copied files are added to the Talkie target
3. **Update Imports**: Change any TalkieAgent-specific imports to Talkie equivalents
...
**Reference Implementation:**
The components already exist in TalkieAgent's onboarding (search for "OnboardingPillDemo" and "CelebrationView")
...
- ⚠️ Ensure .recordingDidStart is posted by TalkieAgent
...
### Components to Port from TalkieAgent:
- PillDemoAnimation
- WaveformDemoView
