...
`apps/macos/Talkie/Interstitial/` - Floating editor panel

Triggered via `talkie://interstitial/{id}` URL from TalkieAgent (Shift-click to stop recording).
...
**Flow:** TalkieAgent Shift-click → URL scheme → InterstitialManager.show() → Edit/Polish → Paste/Copy
...
**URL Scheme Flow:**
1. TalkieAgent records with Shift held
