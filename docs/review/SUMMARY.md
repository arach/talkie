...
| **TalkieAgent** | ~28,000 | 40+ | Background helper for live dictation |
...
| `TalkieAgent/SettingsView.swift` | 4,274 | CRITICAL | All settings in one view |
| `TalkieAgent/DebugKit.swift` | 4,099 | HIGH | Monolithic debug tools |
...
| `TalkieAgent/OnboardingView.swift` | 2,263 | HIGH | Multi-step wizard |
...
| File | Lines | Risk | Issue |
|------|-------|------|-------|
| `TalkieAgent/HomeView.swift` | 1,846 | MEDIUM | Activity timeline |
...
| `TalkieAgent/LiveController.swift` | 1,195 | HIGH | State machine |
...
### 3. TalkieAgent/SettingsView.swift (4,274 lines) - CRITICAL
...
### XPC Communication
```
Talkie ←→ TalkieAgent (dictation control)
Talkie ←→ TalkieEngine (transcription)
TalkieAgent → TalkieEngine (transcription)
```

**Issue**: Talkie directly pastes text - should route through TalkieAgent (see plan).

### Database Ownership
```
live.sqlite
├── TalkieAgent: READ + WRITE (owns schema)
└── Talkie: READ-ONLY
```
...
3. TalkieAgent/TalkieEngine launched as needed
...
### TalkieAgent/SettingsView.swift (4,274 lines)
...
- TalkieAgent/HomeView.swift (1,846 lines)
...
### DO Split (High Value)
1. **TalkieAgent/SettingsView.swift** → Extract section views (Appearance, Engine, Permissions)
...
| TalkieAgent | Initial | [talkie-agent.md](talkie-agent.md) |
