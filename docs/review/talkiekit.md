...
`apps/macos/TalkieKit/` - Shared components for Talkie apps (19 files)
...
- **LogSource:** Talkie, TalkieAgent, TalkieEngine
...
- TalkieAgentXPCServiceProtocol: state, recording, permissions
- TalkieAgentStateObserverProtocol: callbacks for state, dictations, audio level
...
- Bundle IDs: `to.talkie.agent` vs `to.talkie.agent.dev`
- XPC Services: `to.talkie.agent.xpc` vs `to.talkie.agent.xpc.dev`
...
**Discussion:**
- User defaults shared between Talkie, TalkieAgent, TalkieEngine
