...
`apps/macos/Talkie/Stores/` - State management

---

## Overview

The Stores module provides client-side state management that bridges TalkieAgent's GRDB database with Talkie's UI. Uses incremental sync for efficient updates.
...
- **Performance Metrics (perf prefix):**
  ...
  - `perfInAppMs` - TalkieAgent processing
