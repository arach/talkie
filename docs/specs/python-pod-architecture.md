...
Execution pods run ML models in isolated subprocesses. Talkie no longer ships a native Swift pod runtime, but some workloads may still benefit from a future Python-based escape hatch when in-process inference is the wrong fit.
...
Python pod speaks the same JSON-lines protocol as the earlier native pod design:
...
|-------|------------------|
| MLX (in-process) | Model size only |
...
For now, the embedded Swift runtime handles our needs. The Python escape hatch is ready when we need it.
