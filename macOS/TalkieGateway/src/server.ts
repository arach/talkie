...
import { inferenceRoute, inferenceProvidersRoute } from "./routes/inference";
...
async function main() {
  ...
  await clearLog();
  log.info("TalkieGateway starting...");
  ...
  if (LOCAL_MODE) {
    ...
    if (tailscaleState.status !== "ready" && tailscaleState.status !== "no-peers") {
      log.error("Cannot start gateway: Tailscale is not ready");
  ...
  const server = Bun.serve({
    ...
    async fetch(req, server) {
      ...
      // Clickable URL for logs (cmd+click in terminal)
      const clickableUrl = `http://localhost:${PORT}${path}`;

      log.request(method, clickableUrl);

      const logResponse = (response: Response) => {
        const duration = Math.round(performance.now() - startTime);
        log.info(`${method} ${clickableUrl} → ${response.status} (${duration}ms)`);
        return response;
      };
      ...
      try {
        ...
        // POST /inference - Unified LLM inference
        if (path === "/inference" && method === "POST") {
          return logResponse(await inferenceRoute(req));
        }

        // GET /inference/providers - List available providers
        if (path === "/inference/providers" && method === "GET") {
          return logResponse(await inferenceProvidersRoute(req));
        }

        // Headless mode - Claude CLI direct integration
        // POST /headless - Send message via CLI (works when screen locked)
        if (path === "/headless" && method === "POST") {
          return logResponse(await headlessRoute(req));
        }

        // GET /headless/status - Check CLI availability
        if (path === "/headless/status" && method === "GET") {
          return logResponse(await headlessStatusRoute(req));
        }
  ...
  log.info(`TalkieGateway running at http://${hostname}:${PORT}`);
