 ...
 * @param limit - Maximum number of sessions to return (default: 20, helps with huge project dirs)
 */
 ...
 * @param sessionsPerPath - Max sessions per path (default: 20)
 */
export async function discoverPaths(sessionsPerPath = 20): Promise<PathEntry[]> {
  ...
  try {
    ...
    for (const folder of projectFolders) {
      ...
      try {
        ...
        const transcripts = await findAllTranscripts(projectDir, sessionsPerPath);
