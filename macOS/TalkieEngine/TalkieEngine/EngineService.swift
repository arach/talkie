...
final class EngineService: NSObject, TalkieEngineProtocol {
    ...
    private func transcribeWithParakeet(audioPath: String, modelId: String, trace: TranscriptionTrace) async throws -> TranscriptionResult {
        ...
        // Append low-level pink noise to help the decoder flush final tokens
        // Pink noise is more natural than silence (resembles room tone ASR models expect)
        // Previous approach (echo-tail) caused repeated words; noise padding is clean
        trace.begin("audio_pad")
        let padDuration = 4800  // 300ms at 16kHz
        let noisePad = generatePinkNoise(sampleCount: padDuration)
        samples.append(contentsOf: noisePad)
        trace.end("+\(padDuration) samples (pink noise)")
    ...
    /// Generate low-level pink noise for audio padding
    /// Pink noise is more natural than silence for ASR models (resembles room tone)
    /// Amplitude ~-60dB is imperceptible but gives the model realistic "quiet" context
