      ...
      <Container className="relative py-18 md:py-20 xl:py-24">
        ...
        <div className="mt-4 flex items-start justify-center">
          ...
          <Reveal delay={20} className="relative w-full max-w-[52rem]">
            <div className="panel relative overflow-hidden p-0 grid md:grid-cols-2">
              <div className="pointer-events-none absolute inset-0 bg-tactical-grid opacity-[0.06]" />
              ...
              <div className="p-7 md:p-9">
                ...
                <h3 className="mt-2.5 text-[28px] font-extrabold tracking-[0.02em] uppercase text-zinc-100 leading-[1.08]">
                  Own your
                  <br />
                  intelligence.
                </h3>
              ...
              <div className="relative bg-zinc-950 p-6 md:p-8 bracket">
                ...
                <div className="mt-6 flex items-baseline justify-center gap-3">
                  ...
                  <span className="text-[38px] font-extrabold text-zinc-100">{isFree ? '$0' : `$${launch.toFixed(2)}`}</span>
                </div>
                {savingPct > 0 && (
                ...
                {!showForm ? (
                  <div className="mt-4 flex flex-col items-center gap-2">
                    <button onClick={() => setShowForm(true)} className="w-full rounded-sm bg-zinc-100 text-zinc-950 py-2.5 text-[12px] font-semibold tracking-[0.06em] hover:bg-white transition-colors">Get Your Offer</button>
                    <p className="text-[10px] font-mono uppercase tracking-[0.22em] text-zinc-500">We’ll send a secure purchase link</p>
                  </div>
                ) : (
                  <>
                    <form onSubmit={requestCoupon} className="mt-4 space-y-2" aria-label="Launch Offer Form">
                      ...
                      <button type="submit" className="w-full rounded-sm bg-zinc-100 text-zinc-950 dark:bg-zinc-100 dark:text-zinc-950 py-2.5 text-[12px] font-semibold tracking-[0.06em] hover:bg-white transition-colors" disabled={status === 'sending' || status === 'success'}>
                        {status === 'sending' ? 'Sending…' : status === 'success' ? 'Sent ✓' : 'Get Your Offer'}
                      </button>
                    </form>
